...
-record(tictactree, {
    ...
    level1 :: binary(),
    % an array - but OTP compatibility
    level2 :: any()
}).
...
import_tree(ExportedTree) ->
    ...
    FoldFun = fun({X, EncodedL2SegBin}, L2Array) ->
...
-spec keyto_doublesegment32(binary()) -> {non_neg_integer(), non_neg_integer()}.
%% @doc
...
keyto_segment32({segment_hash, SegmentID, ExtraHash, _AltHash}) when
    is_integer(SegmentID), is_integer(ExtraHash)
->
...
keyto_segment48(BinKey) ->
    <<SegmentID:16/integer, ExtraHash:32/integer, AltHash:32/integer, _Rest/binary>> = crypto:hash(
        md5,
        BinKey
    ),
    {segment_hash, SegmentID, ExtraHash, AltHash}.

-spec generate_segmentfilter_list(list(integer()), tree_size()) -> false | list(integer()).
%% @doc
...
-spec adjust_segmentmatch_list(list(integer()), tree_size(), tree_size()) -> list(integer()).
%% @doc
...
-spec match_segment({integer(), tree_size()}, {integer(), tree_size()}) -> boolean().
%% @doc
...
extract_segment(Segment, TicTacTree) ->
    ...
    Level1Pos =
        (Segment bsr ?L2_BITSIZE) band
            (TicTacTree#tictactree.width - 1),
    ...
    <<PreL2:Level2BytePos/binary, SegLeaf2:HashIntLength/integer, PostL2/binary>> = Level2,
    <<PreL1:Level1BytePos/binary, SegLeaf1:HashIntLength/integer, PostL1/binary>> =
        TicTacTree#tictactree.level1,

    {SegLeaf1, SegLeaf2, {PreL1, Level1BytePos, Level1Pos, HashIntLength, PostL1},
...
replace_segment(L1Hash, L2Hash, L1Extract, L2Extract, TicTacTree) ->
    ...
    Level1Upd = <<PreL1:Level1BytePos/binary, L1Hash:HashIntLength/integer, PostL1/binary>>,
    Level2Upd = <<PreL2:Level2BytePos/binary, L2Hash:HashIntLength/integer, PostL2/binary>>,
...
timing_tester(KeyCount, SegCount, SmallSize, LargeSize) ->
    ...
    io:format(
        user,
        "~nCheck with KeyCount=~w SegCount=~w TreeSizes ~w ~w:~n",
