...
-define(MAX_CACHE_LINES, 31).

% -type index_array() :: array:array().

% To live with OTP16
-type index_array() :: any() | none.
...
-spec prepare_for_index(index_array(), leveled_codec:segment_hash()) -> index_array().
%% @doc
...
add_to_index(LM1Array, L0Index, CacheSlot) when CacheSlot < 128 ->
    IndexAddFun = fun(Slot, Acc) ->
        Bin0 = array:get(Slot, Acc),
        BinLM1 = array:get(Slot, LM1Array),
        array:set(
            Slot,
            <<Bin0/binary, 0:1/integer, CacheSlot:7/integer, BinLM1/binary>>,
            Acc
        )
    end,
...
add_to_cache(L0Size, {LevelMinus1, MinSQN, MaxSQN}, LedgerSQN, TreeList) ->
    ...
    case LM1Size of
        ...
        _ ->
            if
                MinSQN >= LedgerSQN ->
                    {MaxSQN, L0Size + LM1Size, lists:append(TreeList, [LevelMinus1])}
            end
    end.
...
-spec check_levelzero(tuple(), list(integer()), list()) -> {boolean(), tuple | not_found}.
%% @doc
...
check_slotlist(Key, _Hash, CheckList, TreeList) ->
    SlotCheckFun = fun(SlotToCheck, {Found, KV}) ->
        case Found of
            true ->
                {Found, KV};
            false ->
...
generate_randomkeys(Seqn, Count, Acc, BucketLow, BRange) ->
    ...
    {K, V} = {{o, "Bucket" ++ BNumber, "Key" ++ KNumber, null}, {Seqn, {active, infinity}, null}},
    generate_randomkeys(
        Seqn + 1,
        Count - 1,
        [{K, V} | Acc],
...
compare_method_test() ->
    ...
    io:format(
        "Crude method took ~w microseconds resulting in tree of " ++
            "size ~w~n",
    ...
    Sz1 = length(Q1),
    io:format(
        "Merge method took ~w microseconds resulting in tree of " ++
            "size ~w~n",
...
with_index_test2() ->
    IndexPrepareFun = fun({K, _V}, Acc) ->
        H = leveled_codec:segment_hash(K),
        prepare_for_index(Acc, H)
    end,
    LoadFun = fun(_X, {{LedgerSQN, L0Size, L0TreeList}, L0Idx, SrcList}) ->
    ...
    CheckFun = fun({K, V}, {L0Idx, L0Cache}) ->
        H = leveled_codec:segment_hash(K),
