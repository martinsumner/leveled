...
-spec prepare_for_index(
    array:array(), leveled_codec:segment_hash()
) -> array:array().
%% @doc
...
-spec add_to_cache(
    ...
    list(),
    boolean()
) -> {integer(), integer(), list()} | empty_push.
%% @doc
...
add_to_cache(L0Size, {LM1, MinSQN, MaxSQN}, LedgerSQN, TreeList, Writeable) ->
    case {Writeable, leveled_tree:tsize(LM1)} of
        ...
        {_, LM1Size} ->
            if
                MinSQN >= LedgerSQN ->
                    {MaxSQN, L0Size + LM1Size, [LM1 | TreeList]}
            end
    end.

-spec to_list(
    integer(), fun((pos_integer()) -> leveled_tree:leveled_tree())
) -> list().
%% @doc
...
-spec check_levelzero(
    tuple(), {integer(), integer()}, list(integer()), list()
) ->
    {boolean(), tuple | not_found}.
%% @doc
%% Check for the presence of a given Key in the Level Zero cache, with the
...
generate_randomkeys(Seqn, Count, Acc, BucketLow, BRange) ->
    ...
    {K, V} = {
        {o, "Bucket" ++ BNumber, "Key" ++ KNumber, null},
...
compare_method_test() ->
    ...
    SWb = os:timestamp(),
    Q1 = merge_trees(
        StartKey, EndKey, TreeList, leveled_tree:empty(?CACHE_TYPE)
    ),
    Sz1 = length(Q1),
    io:format(
...
with_index_test2() ->
    ...
    LoadFun =
        fun(_X, {{LedgerSQN, L0Size, L0TreeList}, L0Idx, SrcList}) ->
            ...
            LM1SL = leveled_tree:from_orderedlist(
                lists:ukeysort(1, LM1), ?CACHE_TYPE
            ),
