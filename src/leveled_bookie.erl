...
-type ttl_secs() :: infinity | integer().
...
book_tempput(Pid, Bucket, Key, Object, IndexSpecs, Tag, TTL) when
    is_integer(TTL)
->
...
-spec book_mput(pid(), list(leveled_codec:object_spec()), ttl_secs()) ->
    ok | pause.
%% @doc
%%
%% When the store is being run in head_only mode, batches of object specs may
...
-spec fetch_head(
    leveled_codec:ledger_key(), pid(), ledger_cache(), boolean()
) -> {not_present | leveled_codec:ledger_value(), boolean()}.
%% doc
...
-spec journal_notfound(
    integer(), pid(), leveled_codec:ledger_key(), integer()
) -> {boolean(), integer()}.
...
ttl_test() ->
    ...
    {async, IndexFolder} = book_returnfolder(
        Bookie1,
        {index_query, "Bucket", {FoldKeysFun, []}, {"idx1_bin", "f8", "f9"},
            {false, undefined}}
    ),
    KeyList = IndexFolder(),
    ...
    {async, IndexFolderTR2} = book_returnfolder(
        Bookie2,
        {index_query, "Bucket", {FoldKeysFun, []}, {"idx1_bin", "f7", "f9"},
            {false, Regex}}
    ),
...
foldobjects_vs_hashtree_testto() ->
    ...
    {async, HTFolder3} =
        book_returnfolder(
            Bookie1,
            {foldheads_allkeys, ?STD_TAG, FoldHeadsFun, true, true, false,
                false, false}
        ),
    ...
    {async, HTFolder4} =
        book_returnfolder(
            Bookie1,
            {foldheads_allkeys, ?STD_TAG, FoldHeadsFun2, false, false, false,
                false, false}
        ),
...
folder_cache_test(CacheSize) ->
    ...
    {async, HTFolder1A} =
        book_returnfolder(
            Bookie1,
            {foldobjects_bybucket, ?STD_TAG, "BucketA", all, FoldObjectsFun,
                false}
        ),
    ...
    {async, HTFolder1B} =
        book_returnfolder(
            Bookie1,
            {foldobjects_bybucket, ?STD_TAG, "BucketB", all, FoldObjectsFun,
                true}
        ),
    ...
    {async, HTFolder2A} =
        book_returnfolder(
            Bookie1,
            {foldheads_bybucket, ?STD_TAG, "BucketA", all, FoldHeadsFun, true,
    ...
    {async, HTFolder2B} =
        book_returnfolder(
            Bookie1,
            {foldheads_bybucket, ?STD_TAG, "BucketB", all, FoldHeadsFun, true,
    ...
    {async, HTFolder2C} =
        book_returnfolder(
            Bookie1,
            {foldheads_bybucket, ?STD_TAG, "BucketB", {"Key", <<"$all">>},
                FoldHeadsFun, true, false, false, false, false}
        ),
    KeyHashList2C = HTFolder2C(),
    {async, HTFolder2D} =
        book_returnfolder(
            Bookie1,
            {foldheads_bybucket, ?STD_TAG, "BucketB", {"Key", "Keyzzzzz"},
                FoldHeadsFun, true, true, false, false, false}
        ),
    ...
    CheckSplitQueryFun =
        fun(SplitInt) ->
            ...
            {async, HTFolder2E} =
                book_returnfolder(
                    Bookie1,
                    {foldheads_bybucket, ?STD_TAG, "BucketB",
                        {"Key", SplitIntEnd}, FoldHeadsFun, true, false, false,
                        false, false}
                ),
            ...
            {async, HTFolder2F} =
                book_returnfolder(
                    Bookie1,
                    {foldheads_bybucket, ?STD_TAG, "BucketB",
                        {SplitIntStart, "Key|"}, FoldHeadsFun, true, false,
                        false, false, false}
                ),
...
foldkeys_headonly_tester(ObjectCount, BlockSize, BStr) ->
    ...
    Key_SKL_Compare =
        lists:usort(
            lists:map(fun({add, _B, K, SK, _V}) -> {K, SK} end, ObjSpecs)
        ),
