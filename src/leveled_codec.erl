...
-type object_spec_v1() ::
    {add | remove, v1, key(), key(), key() | null, list(erlang:timestamp()) | undefined, any()}.
...
-type value_fetcher() ::
    {fun((pid(), leveled_codec:journal_key()) -> any()), pid(), leveled_codec:journal_key()}.
...
headkey_to_canonicalbinary({?HEAD_TAG, Bucket, Key, null}) when is_binary(Bucket), is_binary(Key) ->
    <<Bucket/binary, Key/binary>>;
headkey_to_canonicalbinary({?HEAD_TAG, {BucketType, Bucket}, Key, SubKey}) when
    is_binary(BucketType), is_binary(Bucket)
->
    headkey_to_canonicalbinary({?HEAD_TAG, <<BucketType/binary, Bucket/binary>>, Key, SubKey});
...
-spec strip_to_indexdetails(ledger_kv()) -> {integer(), segment_hash(), last_moddate()}.
...
-spec get_tagstrategy(ledger_key() | tag() | dummy, compaction_strategy()) -> compaction_method().
%% @doc
...
create_value_for_journal({Object, KeyChanges}, Compress, Method) when not is_binary(KeyChanges) ->
...
create_value_for_journal({Object, KeyChangeBin}, Compress, Method) ->
    ...
    <<ObjectBin/binary, KeyChangeBin/binary, KeyChangeBinLen:32/integer, TypeCode:8/integer>>.
...
maybe_compress(JournalBin, PressMethod) ->
    ...
    <<JBin0:Length0/binary, KeyChangeLength:32/integer, Type:8/integer>> = JournalBin,
    ...
    case IsCompressed of
        ...
        false ->
            ...
            V0 = {deserialise_object(OBin2, IsBinary, IsCompressed, IsLz4), binary_to_term(KCBin2)},
...
revert_value_from_journal(JournalBin, ToIgnoreKeyChanges) ->
    ...
    case ToIgnoreKeyChanges of
        true ->
            ...
            {deserialise_object(OBin2, IsBinary, IsCompressed, IsLz4), {[], infinity}};
        false ->
            <<OBin2:Length1/binary, KCBin2:KeyChangeLength/binary>> = JBin0,
            {deserialise_object(OBin2, IsBinary, IsCompressed, IsLz4), binary_to_term(KCBin2)}
    end.
...
-spec obj_objectspecs(list(tuple()), integer(), integer() | infinity) -> list(ledger_kv()).
%% @doc
...
gen_indexspec(Bucket, Key, IdxOp, IdxField, IdxTerm, SQN, TTL) ->
    ...
    {to_ledgerkey(Bucket, Key, ?IDX_TAG, IdxField, IdxTerm), {SQN, Status, no_lookup, null}}.
...
return_proxy(Tag, ObjMetadata, InkerClone, JournalRef) ->
    ...
    term_to_binary(
        {proxy_object, HeadBin, Size, {fun leveled_bookie:fetch_value/2, InkerClone, JournalRef}}
    ).
...
-spec generate_ledgerkv(
    ...
    integer(),
    tuple() | infinity
) -> {any(), any(), any(), {{integer(), integer()} | no_lookup, integer()}, list()}.
%% @doc
...
-spec get_last_lastmodification(list(erlang:timestamp()) | undefined) -> pos_integer() | undefined.
%% @doc
...
indexspecs_test() ->
    ...
    ?assertMatch(
        {{i, "Bucket", {"t1_int", 456}, "Key2"}, {1, {active, infinity}, no_lookup, null}},
        lists:nth(1, Changes)
    ),
    ?assertMatch(
        {{i, "Bucket", {"t1_bin", "adbc123"}, "Key2"}, {1, {active, infinity}, no_lookup, null}},
        lists:nth(2, Changes)
    ),
    ?assertMatch(
        {{i, "Bucket", {"t1_bin", "abdc456"}, "Key2"}, {1, tomb, no_lookup, null}},
        lists:nth(3, Changes)
    ).
