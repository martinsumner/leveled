...
-record(manifest, {
    ...
    % list
    pending_deletes,
    % OTP16 does not like defining type
    ...
    blooms :: any()
    % actually a dict but OTP 16 compatability
...
new_manifest() ->
    ...
    SetLowerLevelFun = fun(IDX, Acc) ->
        array:set(IDX, leveled_tree:empty(?TREE_TYPE), Acc)
    end,
    LevelArray1 = lists:foldl(
        SetLowerLevelFun,
        LevelArray0,
...
-spec load_manifest(manifest(), fun(), fun()) -> {integer(), manifest(), list()}.
%% @doc
...
load_manifest(Manifest, LoadFun, SQNFun) ->
    UpdateLevelFun = fun(LevelIdx, {AccMaxSQN, AccMan, AccFL}) ->
        ...
        {max(AccMaxSQN, SQN1), AccMan#manifest{levels = UpdLevels, blooms = UpdBlooms},
            AccFL ++ FileList}
    end,
    lists:foldl(
        UpdateLevelFun,
...
save_manifest(Manifest, RootPath) ->
    ...
    ManBin = term_to_binary(Manifest#manifest{
        snapshots = [],
        pending_deletes = dict:new(),
        min_snapshot_sqn = 0,
        blooms = dict:new()
...
report_manifest_level(Manifest, LevelIdx) ->
    ...
    AccMemFun = fun(MaybeME, {MemAcc, Max}) ->
...
remove_manifest_entry(Manifest, ManSQN, LevelIdx, Entry) ->
    ...
    PendingDeletes = update_pendingdeletes(
        ManSQN,
        Entry,
        Manifest#manifest.pending_deletes
    ),
...
-spec key_lookup(manifest(), integer(), leveled_codec:ledger_key()) -> false | manifest_owner().
%% @doc
...
-spec mergefile_selector(manifest(), integer(), selector_strategy()) -> manifest_entry().
%% @doc
...
release_snapshot(Manifest, Pid) ->
    FilterFun = fun({P, SQN, ST, TO}, {Acc, MinSQN, Found}) ->
        case P of
            Pid ->
                {Acc, MinSQN, true};
            _ ->
    ...
    {SnapList0, MinSnapSQN, Hit} = lists:foldl(
        FilterFun,
        {[], infinity, false},
...
check_for_work(Manifest, Thresholds) ->
    CheckLevelFun = fun({LevelIdx, MaxCount}, {AccL, AccC}) ->
        case LevelIdx > Manifest#manifest.basement of
            true ->
                {AccL, AccC};
            false ->
...
-spec get_manifest_entry({tuple(), manifest_entry()} | manifest_entry()) -> manifest_entry().
%% @doc
...
load_level(LevelIdx, Level, LoadFun, SQNFun) ->
    HigherLevelLoadFun = fun(ME, {L_Out, L_MaxSQN, FileList, BloomL}) ->
        FN = ME#manifest_entry.filename,
        {P, Bloom} = LoadFun(FN, LevelIdx),
        SQN = SQNFun(P),
        {[ME#manifest_entry{owner = P} | L_Out], max(SQN, L_MaxSQN), [FN | FileList], [
            {P, Bloom}
            | BloomL
        ]}
    end,
    LowerLevelLoadFun = fun({EK, ME}, {L_Out, L_MaxSQN, FileList, BloomL}) ->
        FN = ME#manifest_entry.filename,
        {P, Bloom} = LoadFun(FN, LevelIdx),
        SQN = SQNFun(P),
        {[{EK, ME#manifest_entry{owner = P}} | L_Out], max(SQN, L_MaxSQN), [FN | FileList], [
            {P, Bloom}
            | BloomL
        ]}
    end,
    case LevelIdx =< 1 of
        ...
        false ->
            ...
            {leveled_tree:from_orderedlist(L0, ?TREE_TYPE, ?TREE_WIDTH), MaxSQN, Flist, UpdBloomL}
    end.
...
keylookup_manifest_test() ->
    ...
    {_Man7, _Man8, _Man9, _Man10, _Man11, _Man12, Man13} = changeup_setup(Man6),
...
ext_keylookup_manifest_test() ->
    ...
    ok = file:write_file(
        Man7FN,
        <<Pre:RandPos/bitstring, Flipped:1/integer, Rest/bitstring>>
    ),
                ...
                {[],
                    [
                        {manifest_entry, {o_rkv, "Bucket", "Key10", null},
                            {o_rkv, "Bucket", "Key12949", null}, "<0.313.0>", "./16_1_0.sst", none},
                        {manifest_entry, {o_rkv, "Bucket", "Key129490", null},
                            {o_rkv, "Bucket", "Key158981", null}, "<0.315.0>", "./16_1_1.sst",
                            none},
                        {manifest_entry, {o_rkv, "Bucket", "Key158982", null},
                            {o_rkv, "Bucket", "Key188472", null}, "<0.316.0>", "./16_1_2.sst", none}
                    ],
                    {idxt, 1,
                        {{[
                                {{o_rkv, "Bucket1", "Key1", null},
                                    {manifest_entry, {o_rkv, "Bucket", "Key9083", null},
                                        {o_rkv, "Bucket1", "Key1", null}, "<0.320.0>",
                                        "./16_1_6.sst", none}}
                            ]},
                            {1, {{o_rkv, "Bucket1", "Key1", null}, 1, nil, nil}}}},
                    {idxt, 0, {{}, {0, nil}}}, {idxt, 0, {{}, {0, nil}}}, {idxt, 0, {{}, {0, nil}}},
                    {idxt, 0, {{}, {0, nil}}}, {idxt, 0, {{}, {0, nil}}}, {idxt, 0, {{}, {0, nil}}},
                    []}},
            19, [], 0,
