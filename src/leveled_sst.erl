...
-type sst_closed_pointer() ::
    ...
    {next, leveled_pmanifest:manifest_entry(), range_endpoint(), range_endpoint()}.
...
-spec sst_open(string(), string(), sst_options(), non_neg_integer()) ->
    {ok, pid(), {leveled_codec:ledger_key(), leveled_codec:ledger_key()}, binary()}.
%% @doc
...
sst_new(RootPath, Filename, Level, KVList, MaxSQN, OptsSST, IndexModDate) ->
    ...
    case
        gen_fsm:sync_send_event(
            Pid,
            {sst_new, RootPath, Filename, Level, {SlotList, FK}, MaxSQN, OptsSST0, IndexModDate,
                not_counted, self()},
            infinity
        )
    of
    ...
    case SlotList of
        ...
        _ ->
            ...
            case
                gen_fsm:sync_send_event(
                    Pid,
                    {sst_new, RootPath, Filename, Level, {SlotList, FK}, MaxSQN, OptsSST0,
                        IndexModDate, CountOfTombs, self()},
                    infinity
                )
            of
                {ok, {SK, EK}, Bloom} ->
...
sst_newlevelzero(
    ...
    ok = gen_fsm:sync_send_event(
        Pid,
        {sst_newlevelzero, RootPath, Filename, Penciller, MaxSQN, OptsSST0, ?INDEX_MODDATE},
        infinity
    ),
    ok =
...
-spec sst_get(pid(), leveled_codec:ledger_key()) -> leveled_codec:ledger_kv() | not_present.
%% @doc
...
starting({sst_open, RootPath, Filename, OptsSST, Level}, _From, State) ->
    ...
    {reply, {ok, {Summary#summary.first_key, Summary#summary.last_key}, Bloom}, reader,
        UpdState#state{level = Level}};
starting(
    {sst_new, RootPath, Filename, Level, {SlotList, FirstKey}, MaxSQN, OptsSST, IdxModDate,
        CountOfTombs, StartingPID},
    _From,
    State
) ->
    SW = os:timestamp(),
...
starting(
    {sst_newlevelzero, RootPath, Filename, Penciller, MaxSQN, OptsSST, IdxModDate},
    _From,
    State
) ->
    DeferredStartupTuple =
        {RootPath, Filename, Penciller, MaxSQN, OptsSST, IdxModDate},
    {reply, ok, starting, State#state{deferred_startup_tuple = DeferredStartupTuple, level = 0}};
...
starting({sst_returnslot, FetchedSlot, FetchFun, SlotCount}, State) ->
    ...
    case length(FetchedSlots) == SlotCount of
        ...
        false ->
            ReturnFun = fun(NextSlot) ->
                gen_fsm:send_event(
                    Self,
                    {sst_returnslot, NextSlot, FetchFun, SlotCount}
                )
            end,
            FetchFun(length(FetchedSlots) + 1, ReturnFun),
            {next_state, starting, State#state{new_slots = FetchedSlots}}
    end.
...
reader({get_kv, LedgerKey, Hash}, _From, State) ->
    ...
    {reply, Result, reader, UpdState#state{
        timings = UpdTimings0,
...
reader(
    ...
    {SlotsToFetchBinList, SlotsToPoint} = fetch_range(
        StartKey,
        EndKey,
        ScanWidth,
        SegList,
    ...
    case State#state.yield_blockquery of
        true ->
            {reply, {yield, SlotsToFetchBinList, SlotsToPoint, PressMethod, IdxModDate}, reader,
                State};
        false ->
            {L, BIC} =
                binaryslot_reader(
            ...
            {reply, L ++ SlotsToPoint, reader, State#state{blockindex_cache = BlockIdxC0}}
    end;
...
reader({set_for_delete, Penciller}, _From, State) ->
    ...
    {reply, ok, delete_pending, State#state{penciller = Penciller}, ?DELETE_TIMEOUT};
reader(background_complete, _From, State) ->
    Summary = State#state.summary,
    {reply, {ok, State#state.filename, Summary#summary.first_key, Summary#summary.last_key}, reader,
        State};
...
delete_pending(
    ...
    PressMethod = State#state.compression_method,
    IdxModDate = State#state.index_moddate,
    {reply, {yield, SlotsToFetchBinList, SlotsToPoint, PressMethod, IdxModDate}, delete_pending,
        State, ?DELETE_TIMEOUT};
delete_pending({get_slots, SlotList, SegList, LowLastMod}, _From, State) ->
    ...
    {reply, {SlotBins, PressMethod, IdxModDate}, delete_pending, State, ?DELETE_TIMEOUT};
...
expand_list_by_pointer(
    ...
    FoldFun = fun(X, {Pointers, Remainder}) ->
    ...
    ExpPointers = sst_getfilteredslots(
        SSTPid,
        AccPointers,
        SegList,
        LowLastMod
...
expand_list_by_pointer(
    ...
    ExpPointer = sst_getfilteredrange(
        SSTPid,
        StartKey,
        EndKey,
        Width,
...
sst_getfilteredrange(Pid, StartKey, EndKey, ScanWidth, SegList, LowLastMod) ->
    ...
    case
        gen_fsm:sync_send_event(
            Pid,
            {get_kvrange, StartKey, EndKey, ScanWidth, SegList0, LowLastMod},
            infinity
        )
    of
...
-spec sst_getslots(pid(), list(slot_pointer())) -> list(leveled_codec:ledger_kv()).
%% @doc
...
find_pos(<<>>, _Hash, PosList, _Count) ->
    PosList;
find_pos(
    <<1:1/integer, PotentialHit:15/integer, T/binary>>,
    Checker,
    PosList,
    Count
) ->
...
fetch(LedgerKey, Hash, State, Timings0) ->
    ...
    case extract_header(CachedBlockIdx, IdxModDate) of
        none ->
            ...
            {Result, State#state{blockindex_cache = BlockIndexCache}, Timings3};
                ...
                _ ->
                    ...
                    case array:get(CacheHash, FetchCache) of
                        ...
                        _ ->
                            ...
                            {Result, State#state{fetch_cache = FetchCache0}, Timings4}
                    end
            end
    end.
...
write_file(
    ...
    ok = leveled_util:safe_rename(
        ...
        <<FileVersion:8/integer, SlotsLength:32/integer, SummaryLength:32/integer, SlotsBin/binary,
            SummaryBin/binary>>,
        false
    ),
    FinalName.
...
open_reader(Filename, LoadPageCache) ->
    ...
    <<FileVersion:8/integer, SlotsLength:32/integer, SummaryLength:32/integer>> = Lengths,
...
build_table_summary(
    ...
    SummBin0 =
        term_to_binary(
            {Summary, Bloom, lists:reverse(SlotIndex)},
            ?BINARY_SETTINGS
        ),
...
-spec read_table_summary(binary(), not_counted | non_neg_integer()) ->
    {sst_summary(), leveled_ebloom:bloom(), list(tuple()), not_counted | non_neg_integer()}.
%% @doc
...
generate_filenames(RootFilename) ->
    ...
    case Ext of
        [] ->
            {filename:join(Components) ++ ".pnd", filename:join(Components) ++ ".sst"};
        Ext ->
            DN = filename:dirname(RootFilename),
            FP_NOEXT = filename:basename(RootFilename, Ext),
            {filename:join(DN, FP_NOEXT) ++ ".pnd", filename:join(DN, FP_NOEXT) ++ ".sst"}
    end.
...
-spec accumulate_positions(
    ...
    {binary(), non_neg_integer(), list(non_neg_integer()), leveled_codec:last_moddate()}
) -> {binary(), non_neg_integer(), list(non_neg_integer()), leveled_codec:last_moddate()}.
%% @doc
...
accumulate_positions({K, V}, {PosBinAcc, NoHashCount, HashAcc, LMDAcc}) ->
    ...
    case is_integer(PosH1) of
        true ->
            case NoHashCount of
                0 ->
                    {<<1:1/integer, PosH1:15/integer, PosBinAcc/binary>>, 0, [H1 | HashAcc],
                        LMDAcc0};
                N ->
                    % The No Hash Count is an integer between 0 and 127
                    % and so at read time should count NHC + 1
                    NHC = N - 1,
                    {<<1:1/integer, PosH1:15/integer, 0:1/integer, NHC:7/integer,
                            PosBinAcc/binary>>,
                        0, [H1 | HashAcc], LMDAcc0}
            end;
        false ->
...
generate_binary_slot(Lookup, KVL, PressMethod, IndexModDate, BuildTimings0) ->
    ...
    {B1, B2, B3, B4, B5} =
        case length(KVL) of
            L when L =< SideBlockSize ->
                {serialise_block(KVL, PressMethod), <<0:0>>, <<0:0>>, <<0:0>>, <<0:0>>};
            L when L =< 2 * SideBlockSize ->
                {KVLA, KVLB} = lists:split(SideBlockSize, KVL),
                {serialise_block(KVLA, PressMethod), serialise_block(KVLB, PressMethod), <<0:0>>,
                    <<0:0>>, <<0:0>>};
            L when L =< (2 * SideBlockSize + MidBlockSize) ->
                ...
                {serialise_block(KVLA, PressMethod), serialise_block(KVLB, PressMethod),
                    serialise_block(KVLC, PressMethod), <<0:0>>, <<0:0>>};
            L when L =< (3 * SideBlockSize + MidBlockSize) ->
                ...
                {serialise_block(KVLA, PressMethod), serialise_block(KVLB, PressMethod),
                    serialise_block(KVLC, PressMethod), serialise_block(KVLD, PressMethod),
                    <<0:0>>};
    ...
    Header =
        case IndexModDate of
            true ->
                <<B1L:32/integer, B2L:32/integer, B3L:32/integer, B4L:32/integer, B5L:32/integer,
                    LMD:32/integer, PosBinIndex/binary>>;
            false ->
                <<B1L:32/integer, B2L:32/integer, B3L:32/integer, B4L:32/integer, B5L:32/integer,
                    PosBinIndex/binary>>
        end,
    CheckH = hmac(Header),
    SlotBin =
        <<CheckB1P:32/integer, B1P:32/integer, CheckH:32/integer, Header/binary, B1/binary,
            B2/binary, B3/binary, B4/binary, B5/binary>>,
...
pointer_mapfun(Pointer) ->
    ...
    {Slot#slot_index_value.start_position, Slot#slot_index_value.length,
        Slot#slot_index_value.slot_id, SK, EK}.
...
read_slots(
    Handle,
    SlotList,
    {SegList, LowLastMod, BlockIndexCache},
    PressMethod,
    IdxModDate
) ->
                    ...
                    false ->
                        case SegList of
                            ...
                            _SL ->
                                ...
                                % to be filtered
                                FilterFun = fun(KV) -> in_range(KV, SK, EK) end,
...
-spec extract_header(binary() | none, boolean()) -> {binary(), integer(), binary()} | none.
%% @doc
...
binaryslot_get(FullBin, Key, Hash, PressMethod, IdxModDate) ->
    case crc_check_slot(FullBin) of
        {Header, Blocks} ->
            ...
            {fetch_value(PosList, BlockLengths, Blocks, Key, PressMethod), Header};
        crc_wonky ->
            {not_present, none}
    end.

binaryslot_tolist(FullBin, PressMethod, IdxModDate) ->
    BlockFetchFun = fun(Length, {Acc, Bin}) ->
        case Length of
            0 ->
                {Acc, Bin};
            _ ->
...
binaryslot_trimmedlist(
    ...
    BlockCheckFun = fun(Block, {Acc, Continue}) ->
    ...
    case {crc_check_slot(FullBin), SegList} of
        ...
        {{Header, Blocks}, false} ->
            ...
            <<B1L:32/integer, B2L:32/integer, B3L:32/integer, B4L:32/integer, B5L:32/integer>> =
                BlockLengths,
            <<Block1:B1L/binary, Block2:B2L/binary, MidBlock:B3L/binary, Block4:B4L/binary,
                Block5:B5L/binary>> = Blocks,
            BlocksToCheck =
                blocks_required(
                    {StartKey, EndKey},
...
blocks_required({StartKey, EndKey}, [B1, B2, MidBlock, B4, B5], PressMethod) ->
    ...
    filter_blocks_required(
        fetchends_rawblock(MidBlockList),
        {StartKey, EndKey},
...
crc_check_slot(FullBin) ->
    <<CRC32PBL:32/integer, PosBL:32/integer, CRC32H:32/integer, Rest/binary>> = FullBin,
...
block_offsetandlength(BlockLengths, BlockID) ->
    case BlockID of
        ...
        2 ->
            ...
            {B1L, B2L};
        3 ->
            <<B1L:32/integer, B2L:32/integer, B3L:32/integer, _BR/binary>> = BlockLengths,
            {B1L + B2L, B3L};
        4 ->
            <<B1L:32/integer, B2L:32/integer, B3L:32/integer, B4L:32/integer, _BR/binary>> =
                BlockLengths,
            {B1L + B2L + B3L, B4L};
        5 ->
            <<B1L:32/integer, B2L:32/integer, B3L:32/integer, B4L:32/integer, B5L:32/integer>> =
...
fetchends_rawblock(RawBlock) ->
    {element(1, lists:nth(1, RawBlock)), element(1, lists:last(RawBlock))}.
        ...
        false ->
            case Pos < (2 * SideBlockSize + MidBlockSize) of
                ...
                false ->
                    ...
                    {(TailPos div SideBlockSize) + 4, (TailPos rem SideBlockSize) + 1}
            end
    end.
...
-spec merge_lists(list(), sst_options(), boolean()) ->
    {list(), list(), list(binary_slot()), tuple() | null, non_neg_integer() | not_counted}.
%% @doc
%%
...
merge_lists(KVList1, SSTOpts, IdxModDate) ->
    ...
    {[], [],
        ...
        element(1, lists:nth(1, KVList1)), not_counted}.
...
-spec merge_lists(
    ...
    boolean(),
    boolean()
) -> {list(), list(), list(binary_slot()), tuple() | null, non_neg_integer()}.
%% @doc
...
-spec merge_lists(
    ...
    {list(expanded_pointer()), list(expanded_pointer()), list(binary_slot()),
        leveled_codec:ledger_key() | null, non_neg_integer() | not_counted}.
merge_lists(
    KVL1,
    KVL2,
    LI,
...
-spec form_slot(
    ...
    {list(expanded_pointer()), list(expanded_pointer()),
        {lookup | no_lookup, list(leveled_codec:ledger_kv())}, leveled_codec:ledger_key()}.
%% @doc
...
-spec update_buildtimings(erlang:timestamp(), build_timings(), atom()) -> build_timings().
%% @doc
%%
...
form_slot_test() ->
    ...
    Slot = [{{o, "B1", "K5", null}, {5, {active, infinity}, {99234568, 99234567}, {}}}],
    R1 = form_slot(
        [SkippingKV],
        [],
        {true, 99999999},
...
indexed_list_allindexkeys_trimmed_test() ->
    ...
    {O1, none} = binaryslot_trimmedlist(
        FullBin,
        SK1,
        EK1,
        native,
    ...
    {O2, none} = binaryslot_trimmedlist(
        FullBin,
        SK2,
        EK2,
        native,
    ...
    {O3, none} = binaryslot_trimmedlist(
        FullBin,
        SK3,
        EK3,
        native,
...
indexed_list_mixedkeys_bitflip_test() ->
    ...
    <<B1L:32/integer, _B2L:32/integer, _B3L:32/integer, _B4L:32/integer, _B5L:32/integer,
        _LMD:32/integer, PosBin/binary>> = Header,
    ...
    ?assertMatch([], ToList4),
    ?assertMatch([], ToList5),
    {O4, none} = binaryslot_trimmedlist(
        SlotBin4,
        SK1,
        EK1,
        native,
    ...
    {O5, none} = binaryslot_trimmedlist(
        SlotBin4,
        SK1,
        EK1,
        native,
...
simple_persisted_rangesegfilter_tester(SSTNewFun) ->
    ...
    TestFun = fun(StartKey, EndKey, OutList) ->
        RangeKVL =
...
hashmatching_bytreesize_test() ->
    ...
    GenKeyFun = fun(X) ->
        LK =
            {?RIAK_TAG, B, list_to_binary("Key" ++ integer_to_list(X)), null},
...
timings_test() ->
    ...
    ?assertMatch(
        true,
        T4#sst_timings.slot_fetch_time >
...
corrupted_block_fetch_tester(PressMethod) ->
    ...
    <<CheckB1P:32/integer, B1P:32/integer, CheckH:32/integer, Header:HS/binary, B1:B1L/binary,
        B2:B2L/binary, B3:B3L/binary, B4:B4L/binary, B5:B5L/binary>> = SlotBin,

    CorruptB3 = flip_byte(B3, 0, B3L),
    CorruptSlotBin =
        <<CheckB1P:32/integer, B1P:32/integer, CheckH:32/integer, Header/binary, B1/binary,
            B2/binary, CorruptB3/binary, B4/binary, B5/binary>>,

    CheckFun = fun(N, {AccHit, AccMiss}) ->
