...
-spec cdb_directfetch(pid(), list(), key_only | key_size | key_value_check) -> list().
%% @doc
...
-spec cdb_scan(pid(), fun(), any(), integer() | undefined) -> {integer() | eof, any()}.
%% @doc
...
writer(cdb_roll, State) ->
    ok = leveled_iclerk:clerk_hashtablecalc(
        State#state.hashtree,
        State#state.last_position,
        self()
    ),
...
rolling({return_hashtable, IndexList, HashTreeBin}, _From, State) ->
    ...
    {NewHandle, Index, LastKey} = open_for_readonly(
        NewName,
...
rolling({delete_pending, ManSQN, Inker}, State) ->
    {next_state, rolling, State#state{delete_point = ManSQN, inker = Inker, deferred_delete = true}}.
...
reader({get_kv, Key}, _From, State) ->
    ...
    {reply, Result, reader, State#state{timings = UpdTimings0, timings_countdown = CountDown}};
...
reader({delete_pending, ManSQN, Inker}, State) ->
    {next_state, delete_pending, State#state{delete_point = ManSQN, inker = Inker},
        ?DELETE_TIMEOUT}.
...
delete_pending({get_kv, Key}, _From, State) ->
    ...
    {UpdTimings0, CountDown} =
        update_statetimings(UpdTimings, State#state.timings_countdown),
    {reply, Result, delete_pending,
        State#state{timings = UpdTimings0, timings_countdown = CountDown}, ?DELETE_TIMEOUT};
...
handle_sync_event(
    {cdb_scan, FilterFun, Acc, StartPos},
    From,
    StateName,
    State
) ->
...
finished_rolling(CDB) ->
    RollerFun = fun(Sleep, FinishedRolling) ->
...
open_active_file(FileName) when is_list(FileName) ->
    ...
    {LastPosition, {HashTree, LastKey}} = startup_scan_over_file(
        Handle,
        Position
    ),
...
get(FileName, Key, Cache, QuickCheck, BinaryMode, Timings) when is_list(FileName) ->
    {ok, Handle} = file:open(FileName, [binary, raw, read]),
    get(Handle, Key, Cache, QuickCheck, BinaryMode, Timings);
get(Handle, Key, Cache, QuickCheck, BinaryMode, Timings) when is_tuple(Handle) ->
    SW = os:timestamp(),
...
-spec open_for_readonly(string(), term()) -> {file:io_device(), hashtable_index(), term()}.
%% @doc
...
find_lastkey(Handle, IndexCache) ->
    ScanIndexFun = fun(Index, {LastPos, KeyCount}) ->
...
scan_index_returnpositions(Handle, Position, Count, PosList0) ->
    {ok, _} = file:position(Handle, Position),
    AddPosFun = fun({Hash, HPosition}, PosList) ->
        case Hash of
            0 ->
                PosList;
            _ ->
...
-spec startup_scan_over_file(file:io_device(), file_location()) -> {file_location(), any()}.
%% @doc
...
-spec saferead_keyvalue(file:io_device()) -> false | {any(), any(), integer(), integer()}.
%% @doc
...
-spec safe_read_next_keybin(file:io_device(), integer()) -> false | {term(), binary()}.
%% @doc
%% Return the next key or have false returned if there is some sort of
%% potentially expected error (e.g. due to file truncation).  Note that no
%% CRC check has been performed
...
-spec safe_read_next_value(file:io_device(), integer(), binary()) -> binary() | false.
...
read_integerpairs(<<Int1:32, Int2:32, Rest/binary>>, Pairs) ->
    read_integerpairs(
        <<Rest/binary>>,
        Pairs ++ [{endian_flip(Int1), endian_flip(Int2)}]
    ).
...
-spec update_fetchtimings(no_timing | cdb_timings(), integer()) -> no_timing | cdb_timings().
%% @doc
%%
...
-spec update_statetimings(cdb_timings(), integer()) -> {cdb_timings(), integer()}.
%% @doc
%%
...
update_statetimings(Timings, 0) ->
    case Timings#cdb_timings.sample_count of
        SC when SC >= ?TIMING_SAMPLESIZE ->
            leveled_log:log("CDB19", [
                Timings#cdb_timings.sample_count,
...
multi_key_value_to_record(KVList, BinaryMode, LastPosition) ->
    lists:foldl(
        fun({K, V}, {KPosL, Bin, _LK}) ->
            ...
            {[{K, byte_size(Bin) + LastPosition} | KPosL], <<Bin/binary, Bin0/binary>>, K}
        end,
        {[], <<>>, empty},
        KVList
    ).
...
to_slotmap(HashTree, Index) ->
    ...
    ConvertObjFun = fun({Hash, Position}) ->
...
safe_read_test() ->
    ...
    BinToWrite =
        <<FlippedKeyL:32/integer, FlippedValL:32/integer, Key/binary, ValToWrite/binary>>,

    TestCorruptedWriteFun = fun(BitNumber) ->
        <<PreBin:BitNumber/bitstring, Bit:1/integer, PostBin/bitstring>> = BinToWrite,
        BadBit = Bit bxor 1,
        AltBin = <<PreBin:BitNumber/bitstring, BadBit:1/integer, PostBin/bitstring>>,
        file:delete(TestFN),
