    ...
    case
        {
            ...
            InkerOpts#inker_options.source_inker
        }
    of
...
start_from_file(
    ...
    ?IS_DEF(RootPath), ?IS_DEF(SnapTimeout)
->
    ...
    {Manifest, ManifestSQN, JournalSQN, ActiveJournal} =
...
roll_active(ActiveJournal, Manifest, NewSQN, CDBopts, RootPath, ManifestSQN) ->
    case leveled_cdb:cdb_lastkey(ActiveJournal) of
        LastKey when LastKey =/= empty ->
            ...
            Manifest0 =
                leveled_imanifest:append_lastkey(
                    Manifest, ActiveJournal, LastKey
                ),
            ManEntry =
...
foldfile_between_sequence(
    ...
    case leveled_cdb:cdb_scan(CDBpid, FilterFun, InitBatchAcc, StartPos) of
        ...
        {_LastPosition, {AccMinSQN, _AccMaxSQN, BatchAcc}} when
            AccMinSQN >= JournalSQN
        ->
            {AccMinSQN, FoldFun(BatchAcc, Acc)};
