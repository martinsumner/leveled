...
-type value_status() :: current | replaced | missing.
-type filterfun() ::
    fun(
...
-spec ink_loadpcl(
    ...
    fun((any(), leveled_bookie:ledger_cache()) -> leveled_bookie:ledger_cache())
) ->
...
ink_loadpcl(Pid, MinSQN, LoadFun, InitAccFun, BatchFun) ->
    gen_server:call(
        Pid,
        {fold, MinSQN, {LoadFun, InitAccFun, BatchFun},
            leveled_bookie:empty_ledgercache(), as_ink},
        infinity
    ).
...
ink_compactjournal(Pid, Bookie, _Timeout) ->
    ...
    gen_server:call(
        Pid,
        {compact, Bookie, CheckerInitiateFun, CheckerCloseFun,
            CheckerFilterFun},
        infinity
    ).
...
init([LogOpts, InkerOpts]) ->
    ...
    case
        {
            InkerOpts#inker_options.root_path,
...
->
    ...
    {reply, ok, State};
handle_call(roll, _From, State = #state{is_snapshot = Snap}) when
    Snap == false
->
...
handle_call({backup, BackupPath}, _from, State) when
...
handle_call(ShutdownType, From, State) when
    ShutdownType == close; ShutdownType == doom
->
...
build_manifest(
    ...
    {ActiveLowSQN, _FN, ActiveJournal, _LK} = leveled_imanifest:head_entry(
        OpenManifest
    ),
    JournalSQN =
...
fold_from_sequence(_MinSQN, _FoldFuns, Acc, []) ->
    Acc;
fold_from_sequence(MinSQN, FoldFuns, Acc, [{LowSQN, FN, Pid, _LK} | Rest]) when
    LowSQN >= MinSQN
->
...
fold_from_sequence(MinSQN, FoldFuns, Acc, [{_LowSQN, FN, Pid, _LK} | Rest]) ->
    ...
    {NextMinSQN, Acc0} =
        case Rest of
            ...
            [{NextSQN, _NxtFN, _NxtPid, _NxtLK} | _Rest] when
                NextSQN > MinSQN
            ->
                foldfile_between_sequence(
                    MinSQN,
