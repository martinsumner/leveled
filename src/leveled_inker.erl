...
-record(state, {
    ...
    % in seconds
    snap_timeout :: pos_integer() | undefined,
...
ink_loadpcl(Pid, MinSQN, LoadFun, InitAccFun, BatchFun) ->
    gen_server:call(
        Pid,
        {fold, MinSQN, {LoadFun, InitAccFun, BatchFun}, ok, as_ink},
        infinity
    ).
...
ink_compactjournal(Pid, Bookie, _Timeout) ->
    ...
    gen_server:call(
        Pid,
        {compact, Bookie, CheckerInitiateFun, CheckerCloseFun, CheckerFilterFun},
        infinity
    ).
...
init([LogOpts, InkerOpts]) ->
    ...
    leveled_rand:seed(),
    case {InkerOpts#inker_options.root_path, InkerOpts#inker_options.start_snapshot} of
        {undefined, true} ->
            ...
            {Manifest, ActiveJournalDB, JournalSQN} = ink_registersnapshot(SrcInker, self()),
            {ok, #state{
                manifest = Manifest,
...
handle_call(
    {register_snapshot, Requestor},
    _From,
    State = #state{is_snapshot = Snap}
) when Snap == false ->
    Rs = [{Requestor, os:timestamp(), State#state.manifest_sqn} | State#state.registered_snapshots],
    leveled_log:log("I0002", [Requestor, State#state.manifest_sqn]),
    {reply, {State#state.manifest, State#state.active_journaldb, State#state.journal_sqn},
...
handle_call({confirm_delete, ManSQN}, _From, State) ->
    ...
    CheckSQNFun = fun({_R, _TS, SnapSQN}, Bool) ->
    ...
    CheckSnapshotExpiryFun = fun({_R, TS, _SnapSQN}) ->
    ...
    {reply, lists:foldl(CheckSQNFun, true, RegisteredSnapshots0), State#state{
        registered_snapshots = RegisteredSnapshots0
    }};
...
handle_call({trim, PersistedSQN}, _From, State = #state{is_snapshot = Snap}) when Snap == false ->
    Manifest = leveled_imanifest:to_list(State#state.manifest),
...
handle_call({backup, BackupPath}, _from, State) when State#state.is_snapshot == true ->
    SW = os:timestamp(),
    ...
    BackupFun = fun({SQN, FN, PidR, LastKey}, {ManAcc, FTRAcc}) ->
    ...
    RemoveFun = fun(RFN) ->
        ...
        case
            filelib:is_file(RemoveFile) and
    ...
    leveled_imanifest:writer(
        leveled_imanifest:from_list(BackupManifest),
        State#state.manifest_sqn,
...
handle_cast({clerk_complete, ManifestSnippet, FilesToDelete}, State) ->
    ...
    ok = leveled_iclerk:clerk_promptdeletions(
        State#state.clerk,
        NewManifestSQN,
        FilesToDelete
    ),
...
start_from_file(InkOpts) ->
    ...
    {Manifest, ManifestSQN, JournalSQN, ActiveJournal} = build_manifest(
        ManifestFilenames,
        RootPath,
        CDBopts
    ),
...
shutdown_snapshots(Snapshots) ->
    lists:foreach(
        fun({Snap, _TS, _SQN}) -> ok = ink_close(Snap) end,
        Snapshots
    ).
...
get_cdbopts(InkOpts) ->
    ...
    WasteFP =
        case InkOpts#inker_options.waste_retention_period of
            ...
            _WRP ->
                WFP = filepath(
                    InkOpts#inker_options.root_path,
                    journal_waste_dir
                ),
...
build_manifest(
    ...
    ValidManSQNs = sequencenumbers_fromfilenames(
        ManifestFilenames,
        ManifestRegex,
        'MSQN'
    ),
    ...
    {ActiveLowSQN, _FN, ActiveJournal, _LK} = leveled_imanifest:head_entry(OpenManifest),
    JournalSQN =
...
open_all_manifest(Man0, RootPath, CDBOpts) ->
    ...
    case filelib:is_file(CompleteHeadFN) of
        true ->
            ...
            ManToHead = leveled_imanifest:add_entry(
                OpenedTail,
                {HeadSQN, HeadFN, HeadR, LastKey},
                true
            ),
            NewManEntry = start_new_activejournal(
                LastSQN + 1,
                RootPath,
                CDBOpts
            ),
        ...
        false ->
            {ok, HeadW} = leveled_cdb:cdb_open_writer(
                PendingHeadFN,
                CDBOpts
            ),
            leveled_imanifest:add_entry(
...
-spec fold_from_sequence(integer(), {fun(), fun(), fun()}, any(), list()) -> any().
%% @doc
%%
...
fold_from_sequence(_MinSQN, _FoldFuns, Acc, []) ->
    Acc;
fold_from_sequence(MinSQN, FoldFuns, Acc, [{LowSQN, FN, Pid, _LK} | Rest]) when LowSQN >= MinSQN ->
    {NextMinSQN, Acc0} = foldfile_between_sequence(
        MinSQN,
...
build_dummy_journal(KeyConvertF) ->
    ...
    {ok, MF1} = file:open(
        filename:join(ManifestFP, "1.man"),
...
simple_inker_test() ->
    ...
    {ok, Ink1} = ink_start(#inker_options{
        root_path = RootPath,
        cdb_options = CDBopts,
...
empty_manifest_test() ->
    ...
    {ok, Ink2} = ink_start(#inker_options{
        root_path = RootPath,
        cdb_options = CDBopts,
        compression_method = native,
        compress_on_receipt = false
    ...
    % This is the first key - so should have SQN of 1
    ?assertMatch(1, SQN),
