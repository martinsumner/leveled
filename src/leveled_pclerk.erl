...
-record(state, {
    ...
    % OTP 16 does not like type
    pending_deletions = dict:new(),
...
handle_cast({prompt_deletions, ManifestSQN}, State) ->
    {Deletions, UpdD} = return_deletions(
        ManifestSQN,
...
handle_work({SrcLevel, Manifest}, State) ->
    {UpdManifest, EntriesToDelete} = merge(
        SrcLevel,
        Manifest,
    ...
    SWMC = os:timestamp(),
    ok = leveled_penciller:pcl_manifestchange(
        State#state.owner,
        UpdManifest
    ),
...
merge(SrcLevel, Manifest, RootPath, OptsSST) ->
    ...
    SinkList = leveled_pmanifest:merge_lookup(
        Manifest,
        SrcLevel + 1,
    ...
    case Candidates of
        0 ->
            ...
            Man0 = leveled_pmanifest:switch_manifest_entry(
                Manifest,
                NewSQN,
                SrcLevel,
                Src
...
perform_merge(
    ...
    RevertPointerFun = fun({next, ME, _SK}) ->
        ME
    end,
    SinkManifestList = lists:map(RevertPointerFun, SinkList),
    Man0 = leveled_pmanifest:replace_manifest_entry(
        Manifest,
        NewSQN,
        SinkLevel,
        SinkManifestList,
    ...
    Man2 = leveled_pmanifest:remove_manifest_entry(
        Man0,
        NewSQN,
        SrcLevel,
        Src
...
do_merge(KL1, KL2, SinkLevel, SinkB, RP, NewSQN, MaxSQN, OptsSST, Additions) ->
    FileName = leveled_penciller:sst_filename(
        NewSQN,
        SinkLevel,
        length(Additions)
    ),
...
generate_randomkeys(Count, Acc, BucketLow, BRange) ->
    ...
    RandKey = {K, {Count + 1, {active, infinity}, leveled_codec:segment_hash(K), null}},
...
grooming_score_test() ->
    ...
    {ok, PidL3_1, _, _} =
        leveled_sst:sst_newmerge(
            ...
            "1_L3.sst",
            KL1_L3,
            [{HeadK, setelement(2, HeadV, tomb)} | RestKL2],
            false,
            3,
            999999,
            #sst_options{},
    ...
    lists:foreach(
        fun(P) -> leveled_sst:sst_clear(P) end,
...
merge_file_test() ->
    ...
    PointerList = lists:map(
        fun(ME) -> {next, ME, all} end,
        [E2, E3, E4, E5]
    ),
    {Man6, _Dels} =
        perform_merge(
