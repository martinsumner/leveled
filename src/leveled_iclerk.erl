...
% means that 30.0% should be released)
-type key_size() ::
    {{non_neg_integer(), leveled_codec:journal_key_tag(), leveled_codec:ledger_key()},
        non_neg_integer()}.

-type corrupted_test_key_size() ::
    {{non_neg_integer(), leveled_codec:journal_key_tag(), leveled_codec:ledger_key(), null},
        non_neg_integer()}.
...
clerk_compact(Pid, Checker, InitiateFun, CloseFun, FilterFun, Manifest) ->
    gen_server:cast(
        Pid,
        {compact, Checker, InitiateFun, CloseFun, FilterFun, Manifest}
    ).
...
clerk_hashtablecalc(HashTree, StartPos, CDBpid) ->
    {ok, Clerk} = gen_server:start_link(
        ?MODULE,
        [
...
handle_cast(
    ...
    NotRollingFun = fun({_LowSQN, _FN, Pid, _LK}) ->
...
handle_cast(scoring_complete, State) ->
    ...
    case Score > 0.0 of
        true ->
            ...
            FilesToDelete = lists:map(
                fun(C) ->
                    {C#candidate.low_sqn, C#candidate.filename, C#candidate.journal, undefined}
                end,
                BestRun1
            ),
            leveled_log:log("IC002", [length(FilesToDelete)]),
            ok = CloseFun(FilterServer),
            ok = leveled_inker:ink_clerkcomplete(
                State#state.inker,
                ManifestSlice,
                FilesToDelete
            );
...
schedule_compaction(CompactionHours, RunsPerDay, CurrentTS) ->
    ...
    {NextDate, {NextH, NextI}} =
        case MaybeOK of
            [] ->
                ...
                {calendar:gregorian_days_to_date(Tmrw), lists:nth(1, TooEarly)};
            _ ->
    ...
    NextS0 =
        NextI * (IntervalLength * 60) -
            leveled_rand:uniform(IntervalLength * 60),
    NextM = NextS0 div 60,
    NextS = NextS0 rem 60,
    TimeDiff = calendar:time_difference(
        LocalTime,
...
size_comparison_score(
    ...
    FoldFunForSizeCompare = fun(KS, {ActSize, RplSize}) ->
        case KS of
            {{SQN, Type, PK}, Size} ->
                IsJournalEntry =
...
-spec assess_candidates(list(candidate()), score_parameters()) -> {list(candidate()), float()}.
%% @doc
...
assess_for_runlength(RunLength, AllCandidates, Params, Best) ->
    ...
    FoldFun = fun(Offset, {BestRun, BestScore}) ->
...
score_run(Run, {MaxRunLength, MR_CT, SF_CT}) ->
    ...
    RunTotal = lists:foldl(
        fun(Cand, Acc) ->
            Acc + Cand#candidate.compaction_perc
        end,
        0.0,
        Run
    ),
...
sort_run(RunOfFiles) ->
    CompareFun = fun(Cand1, Cand2) ->
        Cand1#candidate.low_sqn =< Cand2#candidate.low_sqn
    end,
...
compact_files(
    ...
    KVCs0 = leveled_cdb:cdb_directfetch(
        SrcJournal,
        PositionList,
        key_value_check
    ),
    ...
    {ActiveJournal1, ManSlice1} = write_values(
        KVCs1,
        CDBopts,
        ActiveJournal0,
        ManSlice0,
...
get_all_positions([HeadRef | RestOfBest], PositionBatches) ->
    ...
    Batches = split_positions_into_batches(
...
schedule_test_bycount(N) ->
    ...
    io:format(
        "Seconds to compaction ~w for count ~w~n",
...
file_gc_test() ->
    State = #state{
        waste_path = "test/test_area/waste/",
...
find_bestrun_test() ->
    ...
    Params = {4, 60.0, 40.0},
    Block1 = [
        #candidate{compaction_perc = 55.0, filename = "a"},
    ...
    Block2 = [
        #candidate{compaction_perc = 38.0, filename = "e"},
    ...
    Block3 = [
        #candidate{compaction_perc = 70.0, filename = "i"},
    ...
    Block4 = [
        #candidate{compaction_perc = 55.0, filename = "m"},
    ...
    Block5 = [
        #candidate{compaction_perc = 60.0, filename = "q"},
...
fetch_testcdb(RP) ->
    ...
    {ok, CDB1} = leveled_cdb:cdb_open_writer(
        FN1,
...
compact_single_file_recovr_test() ->
    {Candidate, LedgerSrv1, LedgerFun1, CompactFP, CDB} = compact_single_file_setup(),
    ...
    ?assertMatch(
        probably,
        leveled_cdb:cdb_keycheck(
            PidR,
            {8, stnd, test_ledgerkey("Key1")}
        )
    ),
    ?assertMatch(
        missing,
    ...
    ?assertMatch(
        ...
        leveled_cdb:cdb_get(
            PidR,
            {1, stnd, test_ledgerkey("Key1")}
        )
    ),
    RKV1 = leveled_cdb:cdb_get(
        PidR,
...
size_score_test() ->
    KeySizeList = [
        ...
        {{5, ?INKT_STND, {?STD_TAG, <<"B">>, <<"Key5">>, <<"Subk1">>}, null}, 164},
