...
-record(scoring_state, {
    filter_fun :: leveled_inker:filterfun(),
...
clerk_new(InkerClerkOpts) ->
    gen_server:start_link(
        ?MODULE, [leveled_log:get_opts(), InkerClerkOpts], []
    ).
...
handle_cast({score_filelist, [Entry | Tail]}, State) ->
    ...
    CpctPerc =
        case
            {
                leveled_cdb:cdb_getcachedscore(JournalP, os:timestamp()),
                leveled_rand:uniform(State#state.score_onein) == 1,
                State#state.score_onein
            }
        of
            {CachedScore, _UseNewScore, ScoreOneIn} when
                CachedScore == undefined; ScoreOneIn == 1
            ->
...
schedule_compaction(CompactionHours, RunsPerDay, CurrentTS) ->
    ...
    {NextDate, {NextH, NextI}} =
        case MaybeOK of
            [] ->
                % Use first interval picked tomorrow if none of selected run
                % times are today
...
size_score_test() ->
    KeySizeList =
        [
            ...
            {
                {5, ?INKT_STND, {?STD_TAG, <<"B">>, <<"Key5">>, <<"Subk1">>},
                    null},
                164
            },
