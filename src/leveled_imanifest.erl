...
-spec append_lastkey(manifest(), pid(), leveled_codec:journal_key()) -> manifest().
%% @doc
...
find_persistedentries(SQN, ManifestAsList) ->
    DropFun = fun({ME_SQN, _FN, _ME_P, _LK}) ->
        ME_SQN > SQN
    end,
...
to_list(Manifest) ->
    FoldFun = fun({_SQNMarker, SubL}, Acc) ->
        Acc ++ SubL
    end,
