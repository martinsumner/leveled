...
bucket_sizestats(SnapFun, Bucket, Tag) ->
    ...
    Runner = fun() ->
        ...
        Acc = leveled_penciller:pcl_fetchkeys(
            LedgerSnap,
            StartKey,
            EndKey,
            AccFun,
...
-spec bucket_list(fun(), leveled_codec:tag(), fun(), any()) -> {async, fun()}.
%% @doc
...
-spec bucket_list(fun(), leveled_codec:tag(), fun(), any(), integer()) -> {async, fun()}.
%% @doc
...
index_query(SnapFun, {StartKey, EndKey, TermHandling}, FoldAccT) ->
    ...
    Runner = fun() ->
        {ok, LedgerSnapshot, _JournalSnapshot} = SnapFun(),
        Folder = leveled_penciller:pcl_fetchkeys(
            LedgerSnapshot,
            StartKey,
            EndKey,
            AccFun,
...
-spec bucketkey_query(fun(), leveled_codec:tag(), any(), fun_and_acc()) -> {async, fun()}.
%% @doc
            ...
            case Tag of
                ?IDX_TAG ->
                    ...
                    {KeyDefFun(Bucket, null, ?IDX_TAG, IdxFld, StartIdx),
                        KeyDefFun(Bucket, null, ?IDX_TAG, IdxFld, EndIdx), EnsureKeyBinaryFun};
                _ ->
                    {StartOKey, EndOKey} = Query,
                    {leveled_codec:to_ledgerkey(Bucket, StartOKey, Tag),
                        leveled_codec:to_ledgerkey(Bucket, EndOKey, Tag), fun(K, H) ->
                            V = {is_hash, H},
...
foldobjects_allkeys(SnapFun, Tag, FoldObjectsFun, sqn_order) ->
    ...
    FilterFun = fun(JKey, JVal, _Pos, Acc, ExtractFun) ->
            ...
            {?INKT_STND, {B, K}} ->
                ...
                case SQN of
                    ...
                    _ ->
                        ...
                        {ToLoop, {MinSQN, MaxSQN, [{B, K, SQN, Obj} | BatchAcc]}}
                end;
            _ ->
                {loop, Acc}
        end
    ...
    Folder = fun() ->
        ...
        BatchFoldFun = fun(BatchAcc, ObjAcc) ->
            ObjFun = fun({B, K, SQN, Obj}, Acc) ->
...
foldobjects(
    ...
    Folder = fun() ->
        ...
        FoldFunGen = fun({StartKey, EndKey}, FoldAcc) ->
        ...
        ListFoldFun = fun(KeyRange, Acc) ->
            Folder = FoldFunGen(KeyRange, Acc),
            Folder()
        end,
        FolderToWrap = fun() -> lists:foldl(ListFoldFun, InitAcc0, KeyRanges) end,
