...
many_put_compare(_Config) ->
    ...
    TicTacQ =
        {tictactree_obj, {o_rkv, <<"Bucket">>, null, null, true}, TreeSize, fun(
                _B, _K
            ) ->
                accumulate
            end},
...
index_compare(_Config) ->
            ...
            Q =
                {tictactree_idx,
                    {BucketBin,
                        ...
                        ST, ET},
                    TreeSize, fun(_B, _K) -> accumulate end},
    ...
    MismatchQ =
        {index_query, BucketBin, {FoldKeysIndexQFun, []},
            {<<"idx2_bin">>, <<"!">>, <<"|">>}, {true, undefined}},
...
basic_headonly_test(ObjectCount, RemoveCount, HeadOnly) ->
    ...
    RunnerDefinition =
        {foldheads_allkeys, h, {FoldFun, InitAcc}, false, false, false, false,
            false},
    {async, Runner1} =
...
load_objectspecs([], _SliceSize, _Bookie) ->
    ok;
load_objectspecs(ObjectSpecL, SliceSize, Bookie) when
    length(ObjectSpecL) < SliceSize
->
