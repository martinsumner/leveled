...
many_put_compare(_Config) ->
    ...
    {B1, K1, V1, S1, MD} = {"Bucket", "Key1.1.4567.4321", "Value1", [], [{"MDK1", "MDV1"}]},
    ...
    TicTacQ =
        {tictactree_obj, {o_rkv, "Bucket", null, null, true}, TreeSize, fun(_B, _K) ->
            accumulate
        end},
    ...
    TreeA = TreeAFolder(),
    io:format(
        "Build tictac tree with 200K objects in ~w~n",
    ...
    io:format(
        "Compare tictac trees with 200K objects  in ~w~n",
        [timer:now_diff(os:timestamp(), SWC0)]
    ),
    io:format(
        "Tree comparison shows ~w different leaves~n",
        [length(SegList0)]
    ),
    AltList =
    ...
    io:format(
        "Tree comparison shows ~w altered leaves~n",
        [length(AltList)]
    ),
    ...
    WrongPartitionTicTacQ =
        {tictactree_obj, {o_rkv, "Bucket", null, null, false}, TreeSize, fun(_B, _K) -> pass end},
    ...
    ExtractClockFun = fun(Key, Value) ->
        {proxy_object, HeadBin, _Size, _FetchFun} = binary_to_term(Value),
        <<?MAGIC:8/integer, ?V1_VERS:8/integer, VclockLen:32/integer, VclockBin:VclockLen/binary,
            _Rest/binary>> = HeadBin,
        case is_binary(Key) of
            true ->
                {Key, lists:sort(binary_to_term(VclockBin))};
            false ->
                {term_to_binary(Key), lists:sort(binary_to_term(VclockBin))}
        end
    end,
    FoldObjectsFun = fun(_Bucket, Key, Value, Acc) ->
    ...
    io:format(
        "Build tictac tree via object fold with " ++
            "presence check and 200K objects in ~w~n",
    ...
    io:format(
        "Found dirty leaves with exportable comparison of ~w~n",
        [DL_ExportFold]
    ),
    true = DL_ExportFold == 1,

    %% Finding differing keys
    FoldKeysFun = fun(SegListToFind) ->
        fun(_B, K, Acc) ->
    ...
    io:format(
        "Finding ~w keys in ~w dirty segments in ~w~n",
        [
            length(SegKeyList),
            length(SegList0),
    ...
    io:format(
        "Tree comparison following delete shows ~w different leaves~n",
        [length(SegList1)]
    ),
    ...
    TreeC0 = TreeC0Folder(),
    io:format(
        "Build tictac tree with 100K objects in ~w~n",
...
index_compare(_Config) ->
    ...
    lists:foreach(
        fun(ObjL) -> testutil:riakload(Book1A, ObjL) end,
        ObjLists
    ),

    % Split nine lists across Book1B to Book1D, three object lists in each
    lists:foreach(
        fun(ObjL) -> testutil:riakload(Book1B, ObjL) end,
        lists:sublist(ObjLists, 1, 3)
    ),
    lists:foreach(
        fun(ObjL) -> testutil:riakload(Book1C, ObjL) end,
        lists:sublist(ObjLists, 4, 3)
    ),
    lists:foreach(
        fun(ObjL) -> testutil:riakload(Book1D, ObjL) end,
        lists:sublist(ObjLists, 7, 3)
    ),

    GetTicTacTreeFun = fun(X, Bookie) ->
        SW = os:timestamp(),
        ST = "!",
        ET = "|",
        Q =
            {tictactree_idx, {BucketBin, "idx" ++ integer_to_list(X) ++ "_bin", ST, ET}, TreeSize,
                fun(_B, _K) -> accumulate end},
        {async, Folder} = leveled_bookie:book_returnfolder(Bookie, Q),
        R = Folder(),
        io:format(
            "TicTac Tree for index ~w took " ++
                "~w microseconds~n",
    ...
    TicTacTree1_Joined = lists:foldl(
    ...
    DL1_0 = leveled_tictac:find_dirtyleaves(
        TicTacTree1_Full,
        TicTacTree1_Joined
    ),
    ...
    % Merge the tree across the partitions
    TicTacTree2_Joined = lists:foldl(
        fun leveled_tictac:merge_trees/2,
        TicTacTree2_P1,
    ...
    % Go compare! Also check we're not comparing empty trees
    DL2_0 = leveled_tictac:find_dirtyleaves(
        TicTacTree2_Full,
        TicTacTree2_Joined
    ),
    EmptyTree = leveled_tictac:new_tree(empty, TreeSize),
    ...
    {TestObj, TestSpc} = testutil:generate_testobject(
        BucketBin,
        term_to_binary("K9.Z"),
        "Value1",
        [IdxSpc],
    ...
    % Merge the tree across the partitions
    TicTacTree3_Joined = lists:foldl(
        fun leveled_tictac:merge_trees/2,
        TicTacTree3_P1,
    ...
    IdxQ1 =
        {index_query, BucketBin, {fun testutil:foldkeysfun/3, []}, {"idx2_bin", "zz", "zz|"},
            {true, undefined}},
    {async, IdxFolder1} = leveled_bookie:book_returnfolder(Book2C, IdxQ1),
    true = IdxFolder1() >= 1,

    DL_3to2B = leveled_tictac:find_dirtyleaves(
        TicTacTree2_P1,
        TicTacTree3_P1
    ),
    DL_3to2C = leveled_tictac:find_dirtyleaves(
        TicTacTree2_P2,
        TicTacTree3_P2
    ),
    DL_3to2D = leveled_tictac:find_dirtyleaves(
        TicTacTree2_P3,
        TicTacTree3_P3
    ),
    io:format(
        "Individual tree comparison found dirty leaves of ~w ~w ~w~n",
    ...
    DL3_0 = leveled_tictac:find_dirtyleaves(
        TicTacTree3_Full,
        TicTacTree3_Joined
    ),
    ...
    FoldKeysIndexQFun = fun(_Bucket, {Term, Key}, Acc) ->
    ...
    MismatchQ =
        {index_query, BucketBin, {FoldKeysIndexQFun, []}, {"idx2_bin", "!", "|"},
            {true, undefined}},
    ...
    SL_Full = MMFldr_2A(),
    io:format(
        "Segment search across both clusters took ~w~n",
    ...
    Diffs =
        lists:subtract(SL_Full, SL_Joined) ++
...
tuplebuckets_headonly(_Config) ->
    ...
    io:format(
        "Loaded an object count of ~w in ~w ms~n",
        [ObjectCount, timer:now_diff(os:timestamp(), SW0) / 1000]
    ),

    CheckHeadFun = fun({add, B, K, null, H}) ->
        {ok, H} =
    ...
    FoldHeadFun = fun(B, {K, null}, V, Acc) ->
        [{add, B, K, null, V} | Acc]
    end,
    SW1 = os:timestamp(),
    ...
    SegList = [
        testutil:get_aae_segment({TB, B1}, K1),
    ...
    io:format(
        "Returned ~w objects using seglist in ~w ms~n",
        [
...
basic_headonly_test(ObjectCount, RemoveCount, HeadOnly) ->
    ...
    ok = load_objectspecs(ObjectSpecL, 32, Bookie1),
    io:format(
        "Loaded an object count of ~w in ~w microseconds with ~w~n",
        [ObjectCount, timer:now_diff(os:timestamp(), SW0), HeadOnly]
    ),

    FoldFun = fun(_B, _K, V, {HashAcc, CountAcc}) ->
        {HashAcc bxor V, CountAcc + 1}
    end,
    InitAcc = {0, 0},

    RunnerDefinition =
        {foldheads_allkeys, h, {FoldFun, InitAcc}, false, false, false, false, false},
    {async, Runner1} =
    ...
    {AccH1, AccC1} = Runner1(),
    io:format(
        "AccH and AccC of ~w ~w in ~w microseconds~n",
    ...
    case HeadOnly of
        with_lookup ->
            ...
            CheckHeadFun = fun({add, SegID, B, K, H}) ->
                {ok, H} =
...
load_objectspecs([], _SliceSize, _Bookie) ->
    ok;
load_objectspecs(ObjectSpecL, SliceSize, Bookie) when length(ObjectSpecL) < SliceSize ->
