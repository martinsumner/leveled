...
application_defined_tag(_Config) ->
    ...
    io:format(
        "Completed with std tag in ~w ms~n",
    ...
    io:format(
        "Completed with app tag but not function in ~w ms~n",
        [timer:now_diff(os:timestamp(), T2) / 1000]
    ),

    ExtractMDFun = fun(Tag, Size, Obj) ->
    ...
    T3 = os:timestamp(),
    application_defined_tag_tester(
        40000,
        ?STD_TAG,
    ...
    io:format(
        "Completed with std tag and override function in ~w ms~n",
    ...
    io:format(
        "Completed with app tag and override function in ~w ms~n",
...
application_defined_tag_tester(KeyCount, Tag, Functions, ExpectMD) ->
    ...
    StartOpts1 = [
        ...
        {log_level, warn},
        {reload_strategy, [{bespoke_tag1, retain}, {bespoke_tag2, retain}]},
...
bespoketag_recalc(_Config) ->
    ...
    ExtractMDFun = fun(bespoke_tag, Size, Obj) ->
        [{index, IL}, {value, _V}] = Obj,
        {{erlang:phash2(term_to_binary(Obj)), Size, {index, IL}}, [os:timestamp()]}
    end,
    CalcIndexFun = fun(bespoke_tag, UpdMeta, PrvMeta) ->
    ...
    LoadFun = fun(Book, MustFind) ->
        fun(I) ->
            testutil:stdload_object(
                Book,
                B0,
                integer_to_binary(I rem KeyCount),
                I,
    ...
    CountC = FolderC(),
    io:format(
        "All index entries ~w present - recalc ok~n",
        [CountC]
    ),
