...
simple_test_withlog(LogLevel, ForcedLogs) ->
    ...
    {ok, {62888926, 60, undefined}} = leveled_bookie:book_head(
        Bookie2,
        "Bucket1",
        "Key2"
    ),
...
many_put_fetch_head(_Config) ->
    ...
    not_found = leveled_bookie:book_sqn(
        Bookie3,
        testutil:get_bucket(TestObject),
        testutil:get_key(TestObject),
        ?STD_TAG
...
bigsst_littlesst(_Config) ->
    ...
    ok = leveled_bookie:book_destroy(Bookie2),
    io:format(
        "Big SST ~w files Little SST ~w files~n",
...
journal_compaction_tester(Restart, WRP) ->
    ...
    testutil:check_forobject(Bookie0, TestObject),
    {B2, K2, V2, Spec2, MD} = {"Bucket2", "Key2", "Value2", [], [{"MDK2", "MDV2"}]},
    {TestObject2, TestSpec2} = testutil:generate_testobject(
        B2,
        K2,
        V2,
        Spec2,
    ...
    case WRP of
        undefined ->
            timer:sleep(10100);
        % wait for delete_pending timeout
        % Wait 2 seconds for files to be deleted
        _ ->
            FindDeletedFilesFun = fun(X, Found) ->
                case Found of
                    true ->
                        Found;
                    false ->
...
fetchput_snapshot(_Config) ->
    ...
    testutil:check_forlist(SnapBookie2, ChkList1),
    io:format(
        "Checked for replacement objects in active bookie" ++
...
load_and_count(JournalSize, BookiesMemSize, PencillerMemSize) ->
    ...
    ok = leveled_bookie:book_close(Bookie2),
    ManifestFP =
        leveled_pmanifest:filepath(
            filename:join(RootPath, ?LEDGER_FP),
            manifest
        ),
...
load_and_count_withdelete(_Config) ->
    ...
    lists:foldl(
        fun(_X, Acc) ->
            ...
            {_S, Count} = testutil:check_bucket_stats(
                Bookie1,
                "Bucket"
            ),
            if
...
space_clear_ondelete(_Config) ->
    ...
    ok =
        case length(KL2) of
            0 ->
                io:format(
                    "Key list took ~w microseconds for no keys~n",
    ...
    io:format(
        "FNsC - Bookie has ~w ledger files " ++
            "after close~n",
        [length(FNsC_L)]
    ),
    ...
    io:format(
        "Will require the penciller to resolve the issue of creating" ++
            " an empty file as all keys compact on merge~n"
    ),

    CheckFun = fun(X, FileCount) ->
        case FileCount of
            0 ->
                0;
            _ ->
                ...
                io:format(
                    "Looping with ledger file count ~w~n",
                    [length(NewFC)]
                ),
    ...
    io:format(
        "FNsD - Bookie has ~w ledger files " ++
            "after second close~n",
        [length(strip_nonsst(FNsD_L))]
    ),
    lists:foreach(
        fun(FN) ->
...
is_empty_test(_Config) ->
    ...
    {B1, K1, V1, Spec, MD} =
        {term_to_binary("Bucket1"), term_to_binary("Key1"), "Value1", [], [{"MDK1", "MDV1"}]},
    {TestObject1, TestSpec1} =
        testutil:generate_testobject(B1, K1, V1, Spec, MD),
    {B1, K2, V2, Spec, MD} =
        {term_to_binary("Bucket1"), term_to_binary("Key2"), "Value2", [], [{"MDK1", "MDV1"}]},
    {TestObject2, TestSpec2} =
        testutil:generate_testobject(B1, K2, V2, Spec, MD),
    {B2, K3, V3, Spec, MD} =
        {term_to_binary("Bucket2"), term_to_binary("Key3"), "Value3", [], [{"MDK1", "MDV1"}]},
...
safereaderror_startup(_Config) ->
    ...
    K1 =
        <<38, 50, 201, 47, 167, 125, 57, 232, 84, 38, 14, 114, 24, 62, 12, 74>>,
    Obj1 =
        <<87, 150, 217, 230, 4, 81, 170, 68, 181, 224, 60, 232, 4, 74, 159, 12, 156, 56, 194, 181,
            18, 158, 195, 207, 106, 191, 80, 111, 100, 81, 252, 248>>,
    Obj2 =
        <<86, 201, 253, 149, 213, 10, 32, 166, 33, 136, 42, 79, 103, 250, 139, 95, 42, 143, 161, 3,
            185, 74, 149, 226, 232, 214, 183, 64, 69, 56, 167, 78>>,
