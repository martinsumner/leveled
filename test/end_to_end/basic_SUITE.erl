...
many_put_fetch_head(_Config) ->
    ...
    testutil:check_formissingobject(
        Bookie3, <<"Bookie1">>, <<"MissingKey0123">>
    ),
...
is_empty_test(_Config) ->
    ...
    {B1, K1, V1, Spec, MD} = {
        term_to_binary("Bucket1"), term_to_binary("Key1"), "Value1", [], [
            {"MDK1", "MDV1"}
        ]
    },
    ...
    {B1, K2, V2, Spec, MD} = {
        term_to_binary("Bucket1"), term_to_binary("Key2"), "Value2", [], [
            {"MDK1", "MDV1"}
        ]
    },
    {TestObject2, TestSpec2} =
    ...
    {B2, K3, V3, Spec, MD} = {
        term_to_binary("Bucket2"), term_to_binary("Key3"), "Value3", [], [
            {"MDK1", "MDV1"}
        ]
    },
...
safereaderror_startup(_Config) ->
    ...
    Obj1 =
        <<87, 150, 217, 230, 4, 81, 170, 68, 181, 224, 60, 232, 4, 74, 159, 12,
            156, 56, 194, 181, 18, 158, 195, 207, 106, 191, 80, 111, 100, 81,
            252, 248>>,
    Obj2 =
        <<86, 201, 253, 149, 213, 10, 32, 166, 33, 136, 42, 79, 103, 250, 139,
            95, 42, 143, 161, 3, 185, 74, 149, 226, 232, 214, 183, 64, 69, 56,
            167, 78>>,
...
bigpcl_bucketlist(_Config) ->
    ...
    BucketList =
        lists:map(
            fun(I) -> list_to_binary(integer_to_list(I)) end,
    ...
    io:format(
        "Fold pre-close ~w ms post-close ~w ms~n",
