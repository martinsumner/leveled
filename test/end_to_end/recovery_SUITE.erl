...
same_key_rotation_withindexes(_Config) ->
    ...
    Sequence =
        lists:map(
            fun(K) -> lists:map(fun(I) -> {K, I} end, lists:seq(1, IdxCnt)) end,
            lists:seq(1, KeyCnt)
        ),
    ObjList =
        lists:map(
            fun({K, I}) -> ObjectGenFun(K, I) end, lists:flatten(Sequence)
        ),
        ...
        fun(Bookie) ->
            {async, R} =
                leveled_bookie:book_indexfold(
                    ...
                    {Bucket, <<>>},
                    {FoldKeysFun, []},
                    {list_to_binary("binary_bin"), <<0:32/integer>>, <<
                        255:32/integer
                    >>},
                    {true, undefined}
                ),
            ...
            BadAnswers =
                lists:filter(
                    fun({I, _K}) -> I =/= <<IdxCnt:32/integer>> end, QR
                ),
            io:format(
                "Results ~w BadAnswers ~w~n",
...
rotate_wipe_compact(Strategy1, Strategy2) ->
    ...
    Q2 =
        fun(RT) ->
            {index_query, <<"AltBucket6">>, {fun testutil:foldkeysfun/3, []},
                {<<"idx1_bin">>, <<"#">>, <<"|">>}, {RT, undefined}}
        end,
...
recovr_strategy(_Config) ->
    ...
    Q =
        fun(RT) ->
            {index_query, <<"Bucket6">>, {fun testutil:foldkeysfun/3, []},
                {<<"idx1_bin">>, <<"#">>, <<"|">>}, {RT, undefined}}
        end,
...
simple_cachescoring(_Config) ->
    ...
    io:format(
        "Re-opened bookie withour caching - re-compare compaction time~n"
    ),
    io:format("Scoring for first time - every file should need scoring~n"),
    ...
    io:format(
        "With caching ~w first run ~w average other runs~n",
        [TC0 div 1000, AvgSecondRunCache]
    ),
    io:format(
        "Without caching ~w first run ~w average other runs~n",
