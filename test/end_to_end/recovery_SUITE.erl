...
recovery_with_samekeyupdates(_Config) ->
    ...
    % 3 minutes
    AcceptableDuration = 180,
    % Used to track time for overall job
    E2E_SW = os:timestamp(),
    ...
    CheckFun1 = fun(_I) -> ok = testutil:checkhead_forlist(Book1, FinalObjRL) end,
    ...
    io:format("Check for presence of repeated objects~n"),
    % Fetch repeated objects 200 times each
    CheckFun2 = fun(_I) -> ok = testutil:checkhead_forlist(Book2, FinalObjRL) end,
...
rotate_wipe_compact(Strategy1, Strategy2) ->
    ...
    ok = restart_from_blankledger(BookOpts, [
    ...
    ok = restart_from_blankledger(BookOptsAlt, [
        {"Bucket3", Spcl3, LastV3},
        {"Bucket4", Spcl4, LastV4},
        {"Bucket5", Spcl5, LastV5},
    ...
    Q2 = fun(RT) ->
        {index_query, "AltBucket6", {fun testutil:foldkeysfun/3, []}, {"idx1_bin", "#", "|"},
            {RT, undefined}}
    end,
    ...
    DeleteFun = fun({DK, [{add, DIdx, DTerm}]}) ->
        ok = testutil:book_riakdelete(
            Book3,
            "AltBucket6",
            DK,
            [{remove, DIdx, DTerm}]
...
stdtag_recalc(_Config) ->
    ...
    LoadFun = fun(Book) ->
        fun(I) ->
            testutil:stdload_object(
                Book,
                B0,
                erlang:phash2(I rem KeyCount),
                I,
                erlang:phash2({value, I}),
                infinity,
                ?STD_TAG,
                false,
                false
    ...
    CountFold = fun(Book, CurrentCount) ->
    ...
    CountA = FolderA(),
    io:format(
        "Counted double index entries ~w - everything loaded OK~n",
        [CountA]
    ),
    ...
    CountB = FolderB(),
    io:format(
        "Maybe counted less index entries ~w - everything not loaded~n",
        [CountB]
    ),
    ...
    leveled_penciller:clean_testdir(
        proplists:get_value(root_path, BookOpts) ++
            "/ledger"
    ),
    ...
    CountC = FolderC(),
    io:format(
        "Missing index entries ~w - recalc not supported on ?STD_TAG~n",
        [CountC]
    ),
...
recovr_strategy(_Config) ->
    ...
    Q = fun(RT) ->
        {index_query, "Bucket6", {fun testutil:foldkeysfun/3, []}, {"idx1_bin", "#", "|"},
            {RT, undefined}}
    end,
...
aae_missingjournal(_Config) ->
    ...
    FoldHeadsFun = fun(B, K, _V, Acc) -> [{B, K} | Acc] end,
    ...
    {async, AllHeadF2} =
        leveled_bookie:book_returnfolder(
            Bookie2,
            {foldheads_allkeys, ?RIAK_TAG, FoldHeadsFun, true, true, false, false, false}
        ),
...
aae_bustedjournal(_Config) ->
    ...
    20001 = HeadCount,
    GetCount = lists:foldl(
        fun({B, K}, Acc) ->
            case
                testutil:book_riakget(
                    Bookie2,
                    B,
                    K
                )
    ...
    {async, HashTreeF1} = leveled_bookie:book_returnfolder(
        Bookie2,
        {hashlist_query, ?RIAK_TAG, false}
    ),
    KeyHashList1 = HashTreeF1(),
    20001 = length(KeyHashList1),
    {async, HashTreeF2} = leveled_bookie:book_returnfolder(
        Bookie2,
        {hashlist_query, ?RIAK_TAG, true}
    ),
    ...
    SW = os:timestamp(),
    {async, HashTreeF3} = leveled_bookie:book_returnfolder(
        Bookie2,
        {foldobjects_allkeys, ?RIAK_TAG, FoldObjectsFun, false}
    ),
    ...
    true = Delta < 1001,
    io:format(
        "Fetch of hashtree using fold objects took ~w microseconds" ++
    ...
    SW4 = os:timestamp(),
    {async, HashTreeF4} = leveled_bookie:book_returnfolder(
        Bookie3,
        {foldobjects_allkeys, ?RIAK_TAG, FoldObjectsFun, false}
    ),
    ...
    SW5 = os:timestamp(),
    {async, HashTreeF5} = leveled_bookie:book_returnfolder(
        Bookie4,
        {foldobjects_allkeys, ?RIAK_TAG, FoldObjectsFun, false}
    ),
    ...
    {async, HashTreeF6} = leveled_bookie:book_returnfolder(
        Bookie4,
        {hashlist_query, ?RIAK_TAG, true}
    ),
    ...
    SW6 = os:timestamp(),
    {async, HashTreeF7} = leveled_bookie:book_returnfolder(
        Bookie5,
        {foldobjects_allkeys, ?RIAK_TAG, FoldObjectsFun, false}
    ),
...
busted_journal_test(MaxJournalSize, PressMethod, PressPoint, Bust) ->
    ...
    lists:foreach(
        fun({_RN, Obj, Spc}) ->
            testutil:book_riakput(Bookie1, Obj, Spc)
        end,
        ObjList1
    ),
...
allkeydelta_journal_multicompact(_Config) ->
    ...
    {KSpcL2, V2} = testutil:put_altered_indexed_objects(
        Bookie1,
        B,
        KSpcL1,
        false
    ...
    {KSpcL3, _V3} = testutil:put_altered_indexed_objects(
        Bookie2,
        B,
        KSpcL2,
        false
    ...
    {KSpcL4, V4} = testutil:put_altered_indexed_objects(
        Bookie3,
        B,
        KSpcL3,
        false
...
recompact_keydeltas(_Config) ->
    ...
    {KSpcL1, _V1} = testutil:put_indexed_objects(Bookie1, B, 24000),
    {KSpcL2, _V2} = testutil:put_altered_indexed_objects(
        Bookie1,
        B,
        KSpcL1,
        false
    ...
    compact_and_wait(Bookie2, 0),
    {KSpcL3, V3} = testutil:put_altered_indexed_objects(
        Bookie2,
        B,
        KSpcL2,
        false
