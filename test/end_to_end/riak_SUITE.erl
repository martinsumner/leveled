...
basic_riak_tester(Bucket, KeyCount) ->
    ...
    IndexGenFun = fun(ListID) ->
        fun() ->
            RandInt = leveled_rand:uniform(IndexCount),
            ID = integer_to_list(ListID),
            [
                {add, list_to_binary("integer" ++ ID ++ "_int"), RandInt},
                {add, list_to_binary("binary" ++ ID ++ "_bin"), <<RandInt:32/integer>>}
            ]
        end
    end,
    ...
    SegList =
        lists:map(
            fun(Obj) -> testutil:get_aae_segment(Obj) end,
    ...
    KLBySeg = HeadR(),
    io:format(
        "SegList Headfold returned ~w heads in ~w ms~n",
        [
            length(KLBySeg),
            timer:now_diff(os:timestamp(), SW_SL0) / 1000
        ]
    ),
    % not too many false answers
    true = length(KLBySeg) < KeyCount div 1000,
    ...
    io:format(
        "SegList Headfold with xcess segments returned ~w heads in ~w ms~n",
        [
            length(KLByXcessSeg),
            timer:now_diff(os:timestamp(), SW_SL1) / 1000
        ]
    ),
    % Still not too many false answers
    true = length(KLByXcessSeg) < KeyCount div 10,
...
fetchclocks_modifiedbetween(_Config) ->
    ...
    RevertFixedBinKey = fun(FBK) ->
        <<$K, $e, $y, KeyNumber:64/integer>> = FBK,
        KeyNumber
    end,
    StoreFoldFun = fun(_B, K, _V, {_LK, AccC}) ->
        {RevertFixedBinKey(K), AccC + 1}
    end,

    KeyRangeFun = fun(StartNumber, EndNumber) ->
        {range, <<"B0">>, {testutil:fixed_bin_key(StartNumber), testutil:fixed_bin_key(EndNumber)}}
    end,

    % Count with max object count
    FoldRangesFun = fun(FoldTarget, ModRange, EndNumber, MaxCount) ->
        fun(_I, {LKN, KC}) ->
            {async, Runner} =
                leveled_bookie:book_headfold(
                    FoldTarget,
    ...
    R1A = lists:foldl(
        FoldRangesFun(Bookie1A, false, 50000, 13000),
        {0, 0},
        lists:seq(1, 4)
    ),
    ...
    R1B = lists:foldl(
        FoldRangesFun(Bookie1B, false, 50000, 13000),
        {0, 0},
        lists:seq(1, 3)
    ),
    ...
    CrudeStoreFoldFun = fun(LowLMD, HighLMD) ->
        fun(_B, K, V, {LK, AccC}) ->
    ...
    io:format(
        "Comparing queries for Obj1 TS range ~w ~w~n",
        [ObjL1StartTS, ObjL1EndTS]
    ),

    PlusFilterStart = os:timestamp(),
    R3A_PlusFilter = lists:foldl(
        FoldRangesFun(
            Bookie1A,
            {ObjL1StartTS, ObjL1EndTS},
            100000,
    ...
    io:format(
        "Filtered query ~w ms and unfiltered query ~w ms~n",
        [PlusFilterTime, NoFilterTime]
    ),
    true = NoFilterTime > PlusFilterTime,

    SimpleCountFun = fun(_B, _K, _V, AccC) -> AccC + 1 end,
...
crossbucket_aae(_Config) ->
    ...
    {ok, Bookie1} = leveled_bookie:book_start(StartOpts1),
    {B1, K1, V1, S1, MD} =
        {<<"Bucket">>, <<"Key1.1.4567.4321">>, <<"Value1">>, [], [{<<"MDK1">>, <<"MDV1">>}]},
    ...
    GenList = [{binary, 2}, {binary, 40002}, {binary, 80002}, {binary, 120002}],
    CLs = testutil:load_objects(
        40000,
        GenList,
        Bookie2,
        TestObject,
...
test_segfilter_query(Bookie, CLs) ->
    ...
    HeadSegmentFolderGen = fun(SegL, BKL) ->
    ...
    io:format(
        "SegList folders returned results of ~w " ++
...
test_singledelta_stores(BookA, BookB, TreeSize, DeltaKey) ->
    ...
    TicTacFolder =
        {foldheads_allkeys, ?RIAK_TAG,
            {fun head_tictac_foldfun/4, {0, leveled_tictac:new_tree(test, TreeSize)}}, false, true,
            false, false, false},
    % tictac query by bucket (should be same result as all stores)
    TicTacByBucketFolder =
        {foldheads_bybucket, ?RIAK_TAG, <<"Bucket">>, all,
            {fun head_tictac_foldfun/4, {0, leveled_tictac:new_tree(test, TreeSize)}}, false, false,
            false, false, false},

    DLs = check_tictacfold(
        BookA,
        BookB,
        TicTacFolder,
        DeltaKey,
    ...
    HeadSegmentFolder =
        {foldheads_allkeys, ?RIAK_TAG, {get_segment_folder(DLs, TreeSize), []}, false, true, false,
            false, false},
    ...
    io:format(
        "Two unfiltered segment list folds took ~w milliseconds ~n",
        [Time_SL0]
    ),
    io:format(
        "Segment lists found of lengths ~w ~w~n",
    ...
    SuperHeadSegmentFolder =
        {foldheads_allkeys, ?RIAK_TAG, {get_segment_folder(DLs, TreeSize), []}, false, true,
            SegFilterList, false, false},
    ...
    io:format(
        "Two filtered segment list folds took ~w milliseconds ~n",
        [Time_SL1]
    ),
    io:format(
        "Segment lists found of lengths ~w ~w~n",
    ...
    SuperHeadSegmentFolderCP =
        {foldheads_allkeys, ?RIAK_TAG, {get_segment_folder(DLs, TreeSize), []}, true, true,
            SegFilterList, false, false},
    ...
    io:format(
        "Two filtered segment list folds " ++
...
head_tictac_foldfun(B, K, PO, {Count, TreeAcc}) ->
    ExtractFun = fun({BBin, KBin}, Obj) ->
        {VC, _Sz, _SC} = summary_from_binary(Obj),
        {<<BBin/binary, KBin/binary>>, lists:sort(VC)}
    end,
    {Count + 1, leveled_tictac:add_kv(TreeAcc, {B, K}, PO, ExtractFun)}.
...
check_tictacfold(BookA, BookB, HeadTicTacFolder, DeltaKey, TreeSize) ->
    ...
    io:format(
        "Fold over keys revealed counts of ~w and ~w~n",
        [CountA, CountB]
    ),

    DLs = leveled_tictac:find_dirtyleaves(BookATree, BookBTree),
    io:format(
        "Found dirty leaves with Riak fold_heads of ~w~n",
        [length(DLs)]
    ),
    case DeltaKey of
        {B1, K1} ->
...
summary_from_binary(ObjBin, ObjSize) ->
    <<?MAGIC:8/integer, 1:8/integer, VclockLen:32/integer, VclockBin:VclockLen/binary,
        SibCount:32/integer, _Rest/binary>> = ObjBin,
...
handoff(_Config) ->
    ...
    GenList = [binary_uuid, binary_uuid, binary_uuid, binary_uuid],
    [CL0, CL1, CL2, CL3] =
    ...
    FoldStObjectsFun = fun(B, K, V, Acc) ->
        [{B, K, erlang:phash2(V)} | Acc]
    end,

    FoldObjectsFun = fun(Book) ->
        fun(B, K, Obj, ok) ->
    ...
    io:format(
        "Handoff to Book2 in key_order took ~w milliseconds ~n",
        [Time_HO2]
    ),
    SW3 = os:timestamp(),
    {async, Handoff3} =
    ...
    io:format(
        "Handoff to Book3 in sqn_order took ~w milliseconds ~n",
        [Time_HO3]
    ),
    SW4 = os:timestamp(),
    {async, Handoff4} =
    ...
    io:format(
        "Handoff to Book4 in sqn_order took ~w milliseconds ~n",
        [Time_HO4]
    ),
    ...
    StdFolder =
        {foldobjects_allkeys, ?STD_TAG, FoldStObjectsFun, true, sqn_order},
...
dollar_key_index(_Config) ->
    ...
    {ok, Bookie1} = leveled_bookie:book_start(
        RootPath,
        2000,
        50000000,
    ...
    IndexGen = fun() -> [] end,
    ObjL1 = testutil:generate_objects(
        1300,
        {fixed_binary, 1},
        [],
        ObjectGen,
...
dollar_bucket_index(_Config) ->
    ...
    testutil:riakload(Bookie1, ObjL1),
    ObjL2 = testutil:generate_objects(
        1700,
        uuid,
        [],
        ObjectGen,
    ...
    ObjL3 = testutil:generate_objects(
        7000,
        uuid,
        [],
        ObjectGen,
...
bigobject_memorycheck(_Config) ->
    ...
    {ok, BookieR} = leveled_bookie:book_start(
        RootPath,
        2000,
        1000000000,
        testutil:sync_strategy()
...
cdb_memory_check() ->
    ...
    io:format(
        "Total binary memory ~w in ~w CDB processes~n",
