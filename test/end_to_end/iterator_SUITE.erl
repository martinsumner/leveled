...
expiring_indexes(_Config) ->
    ...
    I0Count2 = I0Counter2(),
    io:format(
        "Count with index value ~w changed from ~w to ~w~n",
    ...
    io:format(
        "Query returned ~w entries in ~w seconds - 3 queries + 15s wait~n",
        [length(QR1), FoldTime]
    ),
    true = (LoadTime + FoldTime) < Future,
    % add a second
    SleepTime = round((Future - (LoadTime + FoldTime)) * 1000 + 1000),
...
breaking_folds(_Config) ->
    ...
    FoldKeyThrowFun = fun(_B, {Term, Key}, Acc) ->
    ...
    FoldThrowThousandFun = fun(FoldFun) ->
        fun(B, K, PO, Acc) ->
    ...
    ObjL4 = testutil:generate_objects(
        10,
        binary_uuid,
        [],
        ObjectGen,
...
single_object_with2i(_Config) ->
    ...
    TestSpec = [
        {add, list_to_binary("integer_int"), 100},
    ...
    %% pass
    {async, IdxFolder1} = leveled_bookie:book_indexfold(
        Bookie1,
        "Bucket1",
        {fun testutil:foldkeysfun/3, []},
        {list_to_binary("binary_bin"), <<99:32/integer>>, <<101:32/integer>>},
        {true, undefined}
    ),
    R1 = IdxFolder1(),
    ...
    IdxQ2 =
        {index_query, "Bucket1", {fun testutil:foldkeysfun/3, []},
            {list_to_binary("integer_int"), 99, 101}, {true, undefined}},
    ...
    IdxQ3 =
        {index_query, {"Bucket1", "Key1"}, {fun testutil:foldkeysfun/3, []},
            {list_to_binary("integer_int"), 99, 101}, {true, undefined}},
...
small_load_with2i(_Config) ->
    ...
    IdxQ1 =
        {index_query, "Bucket", {fun testutil:foldkeysfun/3, []}, {"idx1_bin", "#", "|"},
            {true, undefined}},
    ...
    IdxQ2 =
        {index_query, {"Bucket", LastKey}, {fun testutil:foldkeysfun/3, []},
            {"idx1_bin", LastTerm, "|"}, {false, undefined}},
    ...
    FoldObjectsFun = fun(B, K, V, Acc) -> [{B, K, erlang:phash2(V)} | Acc] end,

    {async, HTreeF1} = leveled_bookie:book_objectfold(
        Bookie1,
        ?RIAK_TAG,
        {FoldObjectsFun, []},
        false
    ...
    {async, HTreeF2} = leveled_bookie:book_objectfold(
        Bookie1,
        ?RIAK_TAG,
        "Bucket",
        all,
    ...
    {async, HTreeF3} = leveled_bookie:book_objectfold(
        Bookie1,
        ?RIAK_TAG,
        "Bucket",
        {"idx1_bin", "#", "|"},
    ...
    % also includes the test object
    true = 9901 == length(KeyHashList1),
    ...
    {async, BL} = leveled_bookie:book_bucketlist(
        Bookie2,
        ?RIAK_TAG,
...
query_count(_Config) ->
    RootPath = testutil:reset_filestructure(),
    {ok, Book1} = leveled_bookie:book_start(
        RootPath,
        2000,
        50000000,
    ...
    Index1Count = count_termsonindex(
        BucketBin,
        "idx1_bin",
        Book1,
        ?KEY_ONLY
    ...
    {ok, Book2} = leveled_bookie:book_start(
        RootPath,
        1000,
        50000000,
        testutil:sync_strategy()
    ...
    ok =
        case TotalNameByName of
            Index1Count ->
                ok
        end,
    {ok, RegMia} = re:compile("[0-9]+Mia"),
    Query1 =
        {index_query, BucketBin, {fun testutil:foldkeysfun/3, []}, {"idx2_bin", "2000", "2000|"},
            {false, RegMia}},
    {async, Mia2KFolder1} = leveled_bookie:book_returnfolder(Book2, Query1),
    Mia2000Count1 = length(Mia2KFolder1()),
    Query2 =
        {index_query, BucketBin, {fun testutil:foldkeysfun/3, []}, {"idx2_bin", "2000", "2001"},
            {true, undefined}},
    {async, Mia2KFolder2} = leveled_bookie:book_returnfolder(Book2, Query2),
    Mia2000Count2 = lists:foldl(
        fun({Term, _Key}, Acc) ->
    ...
    ok =
        case Mia2000Count2 of
            Mia2000Count1 when Mia2000Count1 > 0 ->
                io:format(
                    "Mia2000 counts match at ~w~n",
                    [Mia2000Count1]
                ),
                ok
        end,
    ...
    Query3 =
        {index_query, BucketBin, {fun testutil:foldkeysfun/3, []}, {"idx2_bin", "1980", "2100"},
            {false, RxMia2K}},
    ...
    [{_RN, Obj9, Spc9}] = testutil:generate_objects(
        1,
        binary_uuid,
        [],
        V9,
    ...
    R9 = lists:map(
        fun({add, IdxF, IdxT}) ->
            Q =
                {index_query, BucketBin, {fun testutil:foldkeysfun/3, []}, {IdxF, IdxT, IdxT},
                    ?KEY_ONLY},
    ...
    Spc9Del = lists:map(
        fun({add, IdxF, IdxT}) -> {remove, IdxF, IdxT} end,
        Spc9
    ),
    ...
    {ok, Book3} = leveled_bookie:book_start(
        RootPath,
        2000,
        50000000,
        testutil:sync_strategy()
    ...
    {ok, Book4} = leveled_bookie:book_start(
        RootPath,
        2000,
        50000000,
        testutil:sync_strategy()
    ...
    FoldBucketsFun = fun(B, Acc) -> sets:add_element(B, Acc) end,
    BucketListQuery = {bucket_list, ?RIAK_TAG, {FoldBucketsFun, sets:new()}},
    {async, BLF1} = leveled_bookie:book_returnfolder(Book4, BucketListQuery),
    SW_QA = os:timestamp(),
    BucketSet1 = BLF1(),
    io:format(
        "Bucket set returned in ~w microseconds",
        [timer:now_diff(os:timestamp(), SW_QA)]
    ),

    true = sets:size(BucketSet1) == 1,

    ObjList10A = testutil:generate_objects(
        5000,
        binary_uuid,
        [],
        V9,
    ...
    ObjList10B = testutil:generate_objects(
        5000,
        binary_uuid,
        [],
        V9,
    ...
    ObjList10C = testutil:generate_objects(
        5000,
        binary_uuid,
        [],
        V9,
    ...
    {ok, Book5} = leveled_bookie:book_start(
        RootPath,
        2000,
        50000000,
        testutil:sync_strategy()
...
count_termsonindex(Bucket, IdxField, Book, QType) ->
    lists:foldl(
        fun(X, Acc) ->
            ...
            ET = ST ++ "|",
            Q = {index_query, Bucket, {fun testutil:foldkeysfun/3, []}, {IdxField, ST, ET}, QType},
            R = leveled_bookie:book_returnfolder(Book, Q),
            {async, Folder} = R,
            Items = length(Folder()),
            io:format(
                "2i query from term ~s on index ~s took " ++
                    "~w microseconds~n",
                [
                    ST,
                    IdxField,
...
foldobjects_bybucket_range(_Config) ->
    ...
    {async, AllFolder} = leveled_bookie:book_objectfold(
        Bookie1,
        ?RIAK_TAG,
        <<"Bucket1">>,
        all,
