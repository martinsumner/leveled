...
expiring_indexes(_Config) ->
    ...
    io:format(
        "Query returned ~w entries in ~w seconds - 3 queries + 10s wait~n",
        [length(QR3), FoldTime]
    ),
...
small_load_with2i(_Config) ->
    ...
    ObjL1 =
        testutil:generate_objects(
            10000, binary_uuid, [], ObjectGen, IndexGen
        ),
    testutil:riakload(Bookie1, ObjL1),
    ...
    BucketObjQ =
        {foldobjects_bybucket, ?RIAK_TAG, <<"Bucket">>, all, {SumIntFun, 0},
            true},
    ...
    {async, BL} = leveled_bookie:book_bucketlist(
        Bookie2, ?RIAK_TAG, {FoldBucketsFun, sets:new()}, all
    ),
    true = sets:size(BL()) == 2,
...
query_count(_Config) ->
    ...
    {TestObject, TestSpec} =
        testutil:generate_testobject(
            BucketBin, term_to_binary("Key1"), <<"Value1">>, [], [
                {<<"MDK1">>, <<"MDV1">>}
            ]
        ),
    ...
    Query3 =
        {index_query, BucketBin, {fun testutil:foldkeysfun/3, []},
            {<<"idx2_bin">>, <<"1980">>, <<"2100">>}, {false, RxMia2K}},
    ...
    Query3PCRE =
        {index_query, BucketBin, {fun testutil:foldkeysfun/3, []},
            {<<"idx2_bin">>, <<"1980">>, <<"2100">>}, {false, RxMia2KPCRE}},
...
capture_and_filter_terms(_Config) ->
    ...
    ObjL1 =
        testutil:generate_objects(
            100000, binary_uuid, [], V1, IndexGen, Bucket
        ),
    testutil:riakload(Book1, ObjL1),
    ...
    QueryPCRE0 =
        {index_query, {Bucket, null}, {fun testutil:foldkeysfun/3, []},
            {IdxName, <<"M">>, <<"Z">>}, {true, WillowLeedsPCRE}},
    ...
    {async, RunnerRE2_10} = leveled_bookie:book_returnfolder(
        Book1, QueryRE2_10
    ),
...
complex_queries(_Config) ->
    ...
    ObjL1 =
        testutil:generate_objects(
            KeyCount, binary_uuid, [], V1, IndexGen, Bucket
        ),
    testutil:riakload(Book1, ObjL1),
...
count_termsonindex(Bucket, IdxField, Book, QType) ->
    lists:foldl(
        fun(X, Acc) ->
            ...
            Q =
                {index_query, Bucket, {fun testutil:foldkeysfun/3, []},
                    {IdxField, ST, ET}, QType},
...
multibucket_fold(_Config) ->
    ...
    ObjL1 =
        testutil:generate_objects(
            13000, binary_uuid, [], ObjectGen, IndexGen, B1
        ),
    testutil:riakload(Bookie1, ObjL1),
    ObjL2 =
        testutil:generate_objects(
    ...
    ObjL3 =
        testutil:generate_objects(
            7000, binary_uuid, [], ObjectGen, IndexGen, B3
        ),
    testutil:riakload(Bookie1, ObjL3),
    ObjL4 =
        testutil:generate_objects(
            23000, binary_uuid, [], ObjectGen, IndexGen, B4
        ),
    testutil:riakload(Bookie1, ObjL4),
